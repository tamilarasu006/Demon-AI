...
pub struct PySecretScanner {
    inner: DEMON_security::SecretScanner,
}
...
impl PySecretScanner {
    ...
    fn new() -> Self {
        Self {
            inner: DEMON_security::SecretScanner::new(),
        }
    }
...
pub struct PyPIIScanner {
    inner: DEMON_security::PIIScanner,
}
...
impl PyPIIScanner {
    ...
    fn new() -> Self {
        Self {
            inner: DEMON_security::PIIScanner::new(),
        }
    }
...
pub struct PyGuardrailsEngine {
    inner: DEMON_security::GuardrailsEngine<DEMON_engine::Engine>,
}
...
impl PyGuardrailsEngine {
    ...
    fn new(
        ...
        let redaction_mode = match mode {
            "redact" => DEMON_security::RedactionMode::Redact,
            "block" => DEMON_security::RedactionMode::Block,
            _ => DEMON_security::RedactionMode::Warn,
        };
        Ok(Self {
            inner: DEMON_security::GuardrailsEngine::new(
                engine,
                redaction_mode,
                scan_input,
                scan_output,
...
pub struct PyAuditLogger {
    inner: parking_lot::Mutex<DEMON_security::AuditLogger>,
}
...
impl PyAuditLogger {
    ...
    fn new(path: Option<&str>) -> PyResult<Self> {
        ...
        let inner = DEMON_security::AuditLogger::new(&db_path)
            .map_err(|e| PyErr::new::<pyo3::exceptions::PyRuntimeError, _>(e.to_string()))?;
        Ok(Self {
...
pub struct PyCapabilityPolicy {
    inner: DEMON_security::CapabilityPolicy,
}
...
impl PyCapabilityPolicy {
    ...
    fn new(default_deny: bool) -> Self {
        Self {
            inner: DEMON_security::CapabilityPolicy::new(default_deny),
        }
    }
...
pub struct PyInjectionScanner {
    inner: DEMON_security::InjectionScanner,
}
...
impl PyInjectionScanner {
    ...
    fn new() -> Self {
        Self {
            inner: DEMON_security::InjectionScanner::new(),
        }
    }
...
pub struct PyRateLimiter {
    inner: DEMON_security::RateLimiter,
}
...
impl PyRateLimiter {
    ...
    fn new(requests_per_minute: u32, burst_size: u32) -> Self {
        Self {
            inner: DEMON_security::RateLimiter::new(
                DEMON_security::RateLimitConfig {
                    requests_per_minute,
                    burst_size,
                    enabled: true,
                },
...
impl PyTaintSet {
    ...
    fn new() -> Self {
        Self {
            inner: DEMON_security::TaintSet::new(),
        }
    }

    fn add(&mut self, label: &str) {
        let taint_label = match label {
            "pii" => DEMON_security::TaintLabel::Pii,
            "secret" => DEMON_security::TaintLabel::Secret,
            "user_private" => DEMON_security::TaintLabel::UserPrivate,
            "external" => DEMON_security::TaintLabel::External,
            _ => DEMON_security::TaintLabel::External,
        };
        // TaintSet is immutable-style; union with a single-label set.
        self.inner = self.inner.union(
            &DEMON_security::TaintSet::from_labels(&[taint_label]),
        );
    }
