...
struct BootPlan {
    ...
    /// ~/.DEMON/config.toml so `DEMON serve` picks it up.
    engine_host: Option<(String, String)>,
    /// Args appended after `uv run DEMON serve --port <port>`.
...
fn boot_plan(cfg: &InferenceConfig, ram_gb: f64) -> BootPlan {
    match cfg.kind {
        ...
        SourceKind::Custom => {
            ...
            // `model` may be empty if the config is malformed; `DEMON serve`
...
/// Find the DEMON project root (contains pyproject.toml).
/// Checks DEMON_ROOT env var, walks up from the executable, then
/// probes common clone locations.
fn find_project_root() -> Option<std::path::PathBuf> {
    // 1. Explicit env var override
    if let Ok(root) = std::env::var("DEMON_ROOT") {
    ...
    let direct = [
        format!("{home}/DEMON"),
        format!("{home}/projects/hazy/DEMON"),
        format!("{home}/projects/DEMON"),
        format!("{home}/src/DEMON"),
        format!("{home}/Documents/DEMON"),
        format!("{home}/Desktop/DEMON"),
        format!("{home}/Developer/DEMON"),
        format!("{home}/dev/DEMON"),
        format!("{home}/Code/DEMON"),
        format!("{home}/code/DEMON"),
        format!("{home}/repos/DEMON"),
        format!("{home}/github/DEMON"),
    ];
    for p in &direct {
    ...
    // 4. Shallow scan: look for DEMON one level inside common parent dirs.
    //    This catches clones like ~/Documents/my-stuff/DEMON without
    ...
    for parent in &scan_parents {
        ...
        if let Ok(entries) = std::fs::read_dir(&parent_path) {
            for entry in entries.flatten() {
                let candidate = entry.path().join("DEMON");
                if candidate.join("pyproject.toml").exists() {
                    return Some(candidate);
                }
                // Also check if the entry itself is DEMON (case-insensitive match)
                if let Some(name) = entry.file_name().to_str() {
                    if name.eq_ignore_ascii_case("DEMON")
...
// ---------------------------------------------------------------------------
// BackendManager — owns the Ollama + DEMON server child processes
// ---------------------------------------------------------------------------
...
/// Rolling buffer holding the most recent ~16 KB of DEMON stderr.
///
/// Populated by a background drainer task spawned at boot so the pipe
/// never fills and back-pressures `DEMON serve`; consumed by the boot
...
struct BackendManager {
    ...
    DEMON: Option<ChildHandle>,
    DEMON_stderr_tail: StderrTail,
}
...
impl Default for BackendManager {
    fn default() -> Self {
        Self {
            ollama: None,
            DEMON: None,
            DEMON_stderr_tail: Arc::new(Mutex::new(Vec::new())),
        }
    }
}
...
impl BackendManager {
    async fn stop_all(&mut self) {
        if let Some(ref mut h) = self.DEMON {
            h.kill().await;
        }
        self.DEMON = None;
...
/// Outcome of waiting for `DEMON serve` to become healthy.
///
...
fn spawn_DEMON_stderr_drainer(mut stderr: tokio::process::ChildStderr, tail: StderrTail) {
...
async fn read_DEMON_stderr_tail(backend: &SharedBackend) -> String {
    let tail = backend.lock().await.DEMON_stderr_tail.clone();
    let bytes = tail.lock().await.clone();
    String::from_utf8_lossy(&bytes).trim().to_string()
}

/// Poll `DEMON serve` health, watching the child process state so we
/// never wait 10 minutes for a process that crashed in the first second.
async fn wait_for_DEMON_health(
    url: &str,
    timeout: Duration,
    ...
    {
        Ok(c) => c,
        Err(_) => return DEMONStartResult::Timeout,
    };
    let deadline = tokio::time::Instant::now() + timeout;
    loop {
        ...
        let exit_status = {
            ...
            match mgr.DEMON.as_mut() {
                Some(h) => h.child.try_wait().ok().flatten(),
                None => None,
            }
        };
        if let Some(status) = exit_status {
            let stderr = read_DEMON_stderr_tail(backend).await;
            return DEMONStartResult::EarlyExit {
                code: status.code(),
                stderr,
            };
        }
        ...
        match client.get(url).send().await {
            Ok(resp) => {
                ...
                if status == reqwest::StatusCode::SERVICE_UNAVAILABLE {
                    ...
                    return DEMONStartResult::ServiceUnavailable(body);
                }
        ...
        if tokio::time::Instant::now() >= deadline {
            return DEMONStartResult::Timeout;
        }
        tokio::time::sleep(Duration::from_millis(500)).await;
    }
}
...
///
/// When the DEMON desktop binary is shipped as an AppImage, the AppImage
...
async fn boot_backend(backend: SharedBackend, status: SharedStatus) {
    ...
    if plan.launch_ollama {
        ...
        // Point `DEMON serve` at the user's endpoint by writing the engine
        // host into ~/.DEMON/config.toml (the env var alone is shadowed by
    ...
    if project_root.is_none() {
        ...
        let target_path = std::path::PathBuf::from(home_dir()).join("DEMON");
        ...
        if target_path.exists() && !target_path.join("pyproject.toml").exists() {
            let mut s = status.lock().await;
            s.error = Some(format!(
                "{} exists but is not a valid DEMON project. \
                 Remove it and relaunch, or set DEMON_ROOT to the correct path.",
                clone_target,
            ));
            return;
        }
        ...
        {
            let mut s = status.lock().await;
            s.detail = "Downloading DEMON (first launch)...".into();
        }
        ...
        let clone_result = tokio::process::Command::new(&git_bin)
            .args([
                ...
                "--depth",
                "1",
                "https://github.com/DEMON/DEMON.git",
                &clone_target,
            ])
            .stdout(std::process::Stdio::null())
        ...
        match clone_result {
            Ok(child) => match child.wait_with_output().await {
                ...
                Ok(output) => {
                    ...
                    let mut s = status.lock().await;
                    s.error = Some(format!(
                        "Failed to download DEMON: {}. \
                         Clone manually: git clone https://github.com/DEMON/DEMON.git {}",
                        stderr.trim(),
                        clone_target,
                    ));
                    return;
    ...
    // already started `DEMON serve` in a terminal and then launched the
    ...
    // override from config instead of hard-coding DEMON_PORT.
    {
        let client = reqwest::Client::builder()
            .timeout(Duration::from_secs(2))
            .build()
            .unwrap();
        match client
            .get(format!("http://127.0.0.1:{}/health", DEMON_PORT))
            .send()
            .await
        {
            ...
            Ok(resp) if resp.status() == reqwest::StatusCode::SERVICE_UNAVAILABLE => {
                ...
                s.error = Some(format!(
                    "An API server is already running on port {} but its \
                     ...
                     `DEMON serve`, wait for it to finish loading and relaunch. \
    ...
    // the user saw no error, the boot continued, `uv run DEMON serve`
    // then ran in an under-provisioned venv, and the user waited the full
    // 600s health-check window before getting "DEMON server did not
    ...
    // Inject cloud API keys from ~/.DEMON/cloud-keys.env
    for (key, value) in read_cloud_keys() {
        cmd.env(&key, &value);
    }
    let DEMON_child = cmd.spawn();
    ...
    match DEMON_child {
        Ok(mut child) => {
            ...
            let mut mgr = backend.lock().await;
            let tail = mgr.DEMON_stderr_tail.clone();
            mgr.DEMON = Some(ChildHandle { child });
            drop(mgr);
            if let Some(stderr) = stderr_handle {
                spawn_DEMON_stderr_drainer(stderr, tail);
            }
        }
        Err(e) => {
            let mut s = status.lock().await;
            s.error = Some(format!(
                "Could not start DEMON server: {}. \
                 Make sure uv is installed (https://astral.sh/uv) and the DEMON repo is cloned at {}",
                e,
                root.display(),
            ));
            return;
    ...
    let server_url = format!("http://127.0.0.1:{}/health", DEMON_PORT);
    match wait_for_DEMON_health(&server_url, Duration::from_secs(600), &backend).await {
        DEMONStartResult::Ready => {}
        DEMONStartResult::ServiceUnavailable(body) => {
            let mut s = status.lock().await;
            s.error = Some(format!(
                "DEMON server is running but the inference engine is not available \
                 (HTTP 503). This usually means the configured model couldn't be loaded.\n\n\
                 Check the server logs, or run 'uv run DEMON serve --port {}{}' \
        ...
        DEMONStartResult::EarlyExit { code, stderr } => {
            ...
            s.error = Some(if stderr.is_empty() {
                format!(
                    "DEMON server exited (code {}) before becoming ready.\n\n\
                     No stderr output. Check that:\n\
                     1. uv is installed ({})\n\
                     2. The DEMON repo is at {}\n\
        ...
        DEMONStartResult::Timeout => {
            let stderr = read_DEMON_stderr_tail(&backend).await;
            let mut s = status.lock().await;
            s.error = Some(if stderr.is_empty() {
                format!(
                    "DEMON server did not become ready within 10 minutes. Check that:\n\
                     1. uv is installed ({})\n\
                     2. The DEMON repo is at {}\n\
                ...
                format!(
                    "DEMON server did not become ready within 10 minutes.\n\nStderr:\n{}",
                    stderr,
                )
            });
            return;
...
fn api_base() -> String {
    format!("http://127.0.0.1:{}", DEMON_PORT)
}
...
#[tauri::command]
async fn run_DEMON_command(args: Vec<String>) -> Result<String, String> {
    let uv_bin = resolve_bin("uv");

    let mut cmd_args = vec!["run".to_string(), "DEMON".to_string()];
    cmd_args.extend(args.iter().cloned());

    let mut cmd = tokio::process::Command::new(&uv_bin);
    cmd.args(&cmd_args);
    // Run from the project root so `uv run DEMON` resolves the DEMON
    // project regardless of the app's launch cwd. In a packaged install the
    // cwd isn't the checkout, so without this `DEMON` isn't found and the
    ...
    if !is_serve {
        ...
        let output = cmd
            .output()
            .await
            .map_err(|e| format!("Failed to launch DEMON: {}", e))?;
    ...
    // `DEMON serve` is a long-running server that never exits. The old code
    ...
    let mut child = cmd
        .spawn()
        .map_err(|e| format!("Failed to launch DEMON serve: {}", e))?;

    let tail: StderrTail = Arc::new(Mutex::new(Vec::new()));
    if let Some(stderr) = child.stderr.take() {
        spawn_DEMON_stderr_drainer(stderr, tail.clone());
    }
    ...
    let url = format!("http://127.0.0.1:{}/health", DEMON_PORT);
    ...
    loop {
        ...
        if let Ok(Some(status)) = child.try_wait() {
            ...
            return Err(format!(
                "DEMON serve exited (code {:?}) before becoming healthy:\n{}",
                status.code(),
                stderr.trim()
            ));
        }
        if let Ok(resp) = client.get(&url).send().await {
            if resp.status().is_success() {
                ...
                return Ok(format!(
                    "DEMON serve is ready on http://127.0.0.1:{}",
                    DEMON_PORT
                ));
            }
        }
        if tokio::time::Instant::now() >= deadline {
            return Err(format!(
                "DEMON serve did not become healthy on port {} within 120s.",
                DEMON_PORT
            ));
        }
        tokio::time::sleep(Duration::from_millis(500)).await;
...
/// Path to the cloud keys file (~/.DEMON/cloud-keys.env).
...
async fn save_cloud_key(key_name: String, key_value: String) -> Result<(), String> {
    ...
    let reload_url = format!("http://127.0.0.1:{}/v1/cloud/reload", DEMON_PORT);
...
// ---------------------------------------------------------------------------
// Inference-source selection (~/.DEMON/inference.json)
// ---------------------------------------------------------------------------
...
/// Path to the inference-source config (~/.DEMON/inference.json).
...
/// Write the custom-endpoint host into ~/.DEMON/config.toml so
/// `DEMON serve` (which reads that file via load_config) points at it.
...
mod native_overlay {
    ...
    pub unsafe fn create(html: &str, api_port: u16) {
        ...
        if Class::get("DEMONOverlayPanel").is_none() {
            let sup = Class::get("NSPanel").unwrap();
            let mut decl = ClassDecl::new("DEMONOverlayPanel", sup).unwrap();
        ...
        if Class::get("DEMONOverlayNavDelegate").is_none() {
            let sup = Class::get("NSObject").unwrap();
            let mut decl = ClassDecl::new("DEMONOverlayNavDelegate", sup).unwrap();
        ...
        if Class::get("DEMONOverlayMsgHandler").is_none() {
            let sup = Class::get("NSObject").unwrap();
            let mut decl = ClassDecl::new("DEMONOverlayMsgHandler", sup).unwrap();
        ...
        let cls = Class::get("DEMONOverlayPanel").unwrap();
        ...
        let hcls = Class::get("DEMONOverlayMsgHandler").unwrap();
        ...
        let nav_cls = Class::get("DEMONOverlayNavDelegate").unwrap();
    ...
    tauri::Builder::default()
        ...
        .setup(move |app| {
            ...
            let health = MenuItemBuilder::with_id("health", "Health: starting...")
                .enabled(false)
                .build(app)?;
            let quit = MenuItemBuilder::with_id("quit", "Quit DEMON").build(app)?;
            ...
            #[cfg(target_os = "macos")]
            unsafe {
                native_overlay::create(include_str!("overlay.html"), DEMON_PORT);
            }
        ...
        .expect("error while building DEMON Desktop")
        .run(move |_app, event| {
...
mod tests {
    ...
    fn failure_message_includes_exit_code_and_tail_and_hint() {
        let msg = format_uv_sync_failure(
            Path::new("/home/u/.DEMON/src"),
            Some(2),
            "error: failed to resolve numpy==2.1.3",
        );
        assert!(msg.contains("exit 2"));
        assert!(msg.contains("/home/u/.DEMON/src"));
