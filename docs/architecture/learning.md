...
!!! note "Backward compatibility"
    The canonical locations are now `DEMON.learning._stubs` (for `RouterPolicy` and `QueryAnalyzer`) and `DEMON.core.types` (for `RoutingContext`). The old `DEMON.intelligence._stubs` import path still works via a backward-compatibility shim, but new code should import from `DEMON.learning._stubs`.
...
```bash
DEMON ask --router learned "What is the capital of France?"
```
...
```python
from DEMON.learning.router import HeuristicRouter, build_routing_context
...
```python
from DEMON.learning.router import build_routing_context
...
```python
from DEMON.learning.trace_policy import TraceDrivenPolicy
...
```python
from DEMON.learning.sft_policy import SFTRouterPolicy
# or via the backward-compat alias:
from DEMON.learning.sft_policy import SFTPolicy
```
...
```python
from DEMON.learning.agent_advisor import AgentAdvisorPolicy
```
...
```python
from DEMON.learning.icl_updater import ICLUpdaterPolicy
```
...
```python
from DEMON.learning.heuristic_reward import HeuristicRewardFunction
...
over DEMON's 5-primitive configuration space. It automates finding optimal
...
The optimization framework has full Rust parity via the `DEMON-learning`
