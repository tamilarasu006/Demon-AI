...
_PYPI_API = "https://pypi.org/pypi/DEMON/json"


def _config_path() -> Path:
    """Resolve the config path, honoring ``DEMON_CONFIG`` like core.config."""
    override = os.environ.get("DEMON_CONFIG")
    if override:
...
# - ``DEMON_NO_UPDATE_CHECK=1`` — project-specific
# - ``CI=true`` — set by every major CI provider, suppresses by default
_OPT_OUT_ENV_VARS = ("DEMON_NO_UPDATE_CHECK",)
...
def _check_disabled() -> bool:
    ...
    # ``DEMON_NO_UPDATE_CHECK=0`` if they want the nudge anyway.
...
def check_for_updates(command_name: str) -> None:
    ...
    Honors ``DEMON_NO_UPDATE_CHECK=1`` and ``CI=true`` — any
...
def _do_check() -> None:
    ...
    try:
        if Version(latest) > Version(current):
            ...
            sys.stderr.write(
                f"\033[33mA new version of DEMON is available "
                f"(v{current} → v{latest})\n"
                f"Update: {cmd}\n"
                f"Or run: DEMON self-update\033[0m\n\n"
            )
