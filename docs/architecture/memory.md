...
- **Persistence:** Data persists across restarts in `~/.DEMON/memory.db`
...
from DEMON.tools.storage.faiss_backend import FAISSMemory
from DEMON.tools.storage.hybrid import HybridMemory
...
!!! note "Backward compatibility"
    The old imports (e.g., `from DEMON.memory.sqlite import SQLiteMemory`) still work via backward-compatibility shims in the `memory/` package, but the canonical location is now `DEMON.tools.storage.*`.
...
```python
from DEMON.tools.storage.chunking import chunk_text, ChunkConfig
...
```python
from DEMON.tools.storage.embeddings import SentenceTransformerEmbedder
...
```python
from DEMON.tools.storage.context import inject_context, ContextConfig
...
The default backend is configured in `~/.DEMON/config.toml`. Storage settings live under `[tools.storage]`, and context injection is controlled by `agent.context_from_memory`:
