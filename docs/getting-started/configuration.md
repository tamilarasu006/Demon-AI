---
title: Configuration
description: Complete reference for DEMON configuration
---

# Configuration

DEMON uses a TOML configuration file to control engine selection, model identity, memory backends, agent behavior, and more. This page is the complete reference for every configuration option, organized by primitive.
...
DEMON creates the `~/.DEMON/` directory and populates it with a default config when you run `DEMON init`.

## Relocating the DEMON directory

DEMON keeps **all** of its state — config, databases, caches, logs,
...
`~/.DEMON`, but you can move it.

The root is resolved in priority order:

1. **`$DEMON_HOME`** — explicit override. Honored by both the installer
   and the Python runtime.
2. **`$XDG_DATA_HOME/DEMON`** — used when `$XDG_DATA_HOME` is set (a single
   `DEMON` directory nested under it, per the XDG Base Directory spec).
3. **`~/.DEMON`** — the default. With no environment variables set, the
   resolved path is exactly this, so existing installs are untouched.

```bash
# Relocate the whole install + runtime tree at install time:
DEMON_HOME=~/apps/DEMON curl -fsSL https://DEMON.github.io/DEMON/install.sh | bash

# Or for a single run / your shell profile:
export DEMON_HOME=~/apps/DEMON
```
...
!!! note "Migration"
    ...
    existing installs. If you set `DEMON_HOME` (or `XDG_DATA_HOME`) on a
    machine that already has data in `~/.DEMON`, DEMON will look in
    the new location and not see your old data — move it yourself if you want
    to keep it: `mv ~/.DEMON "$DEMON_HOME"`.

`$DEMON_CONFIG` still points at an explicit `config.toml` file
...
3. Writes `~/.DEMON/config.toml` with sensible defaults
...
|-------|------|---------|-------------|
| `default` | string | Auto-detected | Default engine backend. One of: `ollama`, `vllm`, `llamacpp`, `sglang`, `cloud`. Set automatically by `DEMON init` based on hardware detection. |
...
!!! tip "Engine fallback"
    If the configured default engine is unreachable, DEMON automatically probes all registered engines and falls back to any healthy one.
...
When both `default_model` and `fallback_model` are empty, DEMON uses the configured router policy (see `[learning]`) to select a model from those available on the active engine.
...
```bash
DEMON ask --router heuristic "Hello"
```
...
| `db_path` | string | `~/.DEMON/memory.db` | Path to the SQLite memory database. Used by the `sqlite` backend. |
...
Controls the OpenAI-compatible API server started by `DEMON serve`.
...
```bash
DEMON serve --host 127.0.0.1 --port 9000 --model qwen3:8b --agent simple
```
...
[traces]
enabled = false
db_path = "~/.DEMON/traces.db"
```

| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `enabled` | bool | `false` | Whether to record traces for each agent interaction. |
| `db_path` | string | `~/.DEMON/traces.db` | Path to the SQLite trace database. |
...
| `skills_dir` | string | `~/.DEMON/skills/` | Directory where skills are installed. |
...
overlay_dir = "~/.DEMON/learning/skills/"
```
...
| `overlay_dir` | string | `~/.DEMON/learning/skills/` | Where optimized skill overlays are stored. |
...
When you run `DEMON init`, DEMON probes your system to detect available hardware. The detection runs in this order:
...
If you have an existing `~/.DEMON/config.toml` from a previous version, here is what changed and how to update it.
...
You can configure DEMON entirely from Python without a TOML file:
...
```python
j = DEMON(config_path="/path/to/my-config.toml")
```
...
DEMON respects the following environment variables:
