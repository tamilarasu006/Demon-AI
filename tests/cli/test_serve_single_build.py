...
1. ``SystemBuilder.build`` is never called during ``DEMON serve`` startup
...
def _repopulate_registries() -> None:
    ...
    if not AgentRegistry.keys():
        for mod_name in list(sys.modules):
            if mod_name.startswith("DEMON.agents.") and not mod_name.endswith(
                "_stubs"
            ):
                try:
    ...
    if not ToolRegistry.keys():
        for mod_name in list(sys.modules):
            if (
                mod_name.startswith("DEMON.tools.")
...
def _run_serve(tmp_path, monkeypatch, *, build_spy, set_system_spy):
    """Invoke ``DEMON serve`` with all heavy/blocking pieces stubbed out.
    ...
    monkeypatch.setattr("DEMON.security.setup_security", lambda *a, **k: sec)

    with (
        patch(
            "DEMON.system.builder.SystemBuilder.build",
            build_spy,
        ),
        patch(
            "DEMON.agents.executor.AgentExecutor.set_system",
            set_system_spy,
        ),
...
def test_serve_does_not_call_systembuilder_build(tmp_path, monkeypatch):
    ...
    build_spy = MagicMock(
        side_effect=AssertionError(
            "SystemBuilder.build() must not run during `DEMON serve` startup "
