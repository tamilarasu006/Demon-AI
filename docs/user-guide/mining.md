...
DEMON can mine the Pearl Proof-of-Useful-Work chain through local LLM
...
DEMON falls back to building from the pinned Pearl source checkout. First
builds can take 30-60 minutes.

On a shared NVIDIA host, restrict the miner to idle GPUs:

```bash
uv run DEMON mine init --cuda-visible-devices 0
```
...
- `DEMON mine models` lists Pearl model support status.
- `DEMON mine inspect-model` checks a Pearl model artifact before GPU launch.
- `DEMON mine doctor` prints hardware, Docker, Pearl node, wallet, provider,
  and session checks.
- `DEMON mine init` writes the local mining config and resolves the image.
- `DEMON mine start` launches the Pearl miner container and writes the runtime
  sidecar.
- `DEMON mine stop` stops the provider and removes the sidecar.
- `DEMON mine status` reads live gateway metrics.
- `DEMON mine attach` writes a sidecar for a miner you launched manually.
- `DEMON mine logs` prints the Docker container log tail.
- `DEMON mine validate-model` probes the active vLLM miner and gateway before
...
DEMON only lists Pearl-compatible models published by the Pearl Research
