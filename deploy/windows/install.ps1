<#
.SYNOPSIS
    DEMON native Windows installer.
...
.DESCRIPTION
    ...
    Steps:
      ...
      5. Clone the DEMON repository to $env:LOCALAPPDATA\DEMON
         (override with $env:DEMON_HOME).
      ...
      7. Optionally register the scheduled-task service (see
         deploy/windows/DEMON-service.ps1).

    Usage (one-liner):
      irm https://DEMON.github.io/DEMON/install.ps1 | iex

    Usage (file invocation, supports flags):
      irm https://DEMON.github.io/DEMON/install.ps1 -OutFile install.ps1
...
.NOTES
    ...
    pass `--host 0.0.0.0` AND set $env:DEMON_API_KEY (an
...
if (-not $SkipService -and $env:DEMON_SKIP_SERVICE) { $SkipService = $true }
if (-not $Service     -and $env:DEMON_SERVICE)      { $Service     = $true }
if (-not $Force       -and $env:DEMON_FORCE)        { $Force       = $true }
...
if ($pyMajor -ne 3 -or $pyMinor -lt 10 -or $pyMinor -gt 13) {
    Write-Fail @"
Found Python $pyMajor.$pyMinor at $pythonExe, but DEMON requires
...
$installRoot = if ($env:DEMON_HOME) {
    $env:DEMON_HOME
} else {
    Join-Path $env:LOCALAPPDATA 'DEMON'
}
...
$modelPullOk = $false
if ($ollamaReady) {
    Write-Info "Pulling qwen3.5:2b (~1.5 GB) so 'DEMON' works on first run..."
...
# ---------------------------------------------------------------------------
# 9. DEMON.cmd shim - so bare `DEMON` works in any new PowerShell
# ---------------------------------------------------------------------------

$binDir = Join-Path $installRoot 'bin'
$shimPath = Join-Path $binDir 'DEMON.cmd'
...
# so the shim is self-locating - moving %LOCALAPPDATA%\DEMON won't
...
setlocal
set "SRC=%~dp0..\src"
uv run --project "%SRC%" DEMON %*
"@
Set-Content -Path $shimPath -Value $shimContent -Encoding ASCII

# Add %LOCALAPPDATA%\DEMON\bin to User PATH if it isn't already
...
# the entry as `%LOCALAPPDATA%\DEMON\bin` (unexpanded) into User
...
Write-Ok "DEMON shim installed at $shimPath"

# ---------------------------------------------------------------------------
# 10. Optional: register the scheduled-task service
# ---------------------------------------------------------------------------

$serviceScript = Join-Path $srcDir 'deploy\windows\DEMON-service.ps1'
...
if ($Service) {
    ...
    if ($isInteractive) {
        $reply = Read-Host "Register DEMON as a Windows scheduled task (auto-start at logon, loopback only)? [y/N]"
...
if ($shouldInstallService) {
    ...
    Write-Ok "Scheduled task 'DEMON' registered (loopback default)."
}
...
Write-Host "  |   DEMON install complete    |" -ForegroundColor Green
Write-Host "  +----------------------------------+" -ForegroundColor Green
Write-Host ""
...
$nextCmd = if ($modelPullOk) { 'DEMON' } else { 'DEMON doctor' }

if ($pathNeedsRefresh) {
    Write-Host ""
    Write-Host "  Run it:  open a NEW PowerShell, then: $nextCmd" -ForegroundColor Yellow
    Write-Host "           (the DEMON shim was added to your User PATH; the"
...
if (-not $modelPullOk) {
    ...
    Write-Host "        'DEMON doctor' shows progress."
}

if ($shouldInstallService) {
    Write-Host ""
    Write-Host "  Service: schtasks /Query /TN DEMON     (status)"
    Write-Host "           powershell -File `"$serviceScript`" uninstall    (remove)"
}
Write-Host ""
Write-Host "  Docs:    https://DEMON.github.io/DEMON/"
Write-Host ""
