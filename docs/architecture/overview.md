...
description: The five-primitive architecture behind DEMON — Intelligence, Engine, Agents, Tools, and Learning
search:
  boost: 2
---
...
DEMON is a research framework for studying on-device AI systems. Its architecture is organized around **five core abstractions** -- Intelligence, Engine, Agentic Logic, Memory, and Learning -- that work together through trace-driven feedback.

![DEMON Architecture](../assets/DEMON_Architecture.png)
...
The sandbox module (`DEMON.sandbox`) adds a `SandboxedAgent` wrapper that runs any `BaseAgent` inside a Docker or Podman container with mount-security enforcement, and a `ContainerRunner` that manages the container lifecycle.
...
All extensible components in DEMON use a **decorator-based registry** for runtime discovery. The pattern is implemented in `RegistryBase[T]`, a generic base class that provides isolated storage per typed subclass.
...
src/DEMON/
    core/               Core infrastructure shared by all primitives
        ...
        config.py           DEMONConfig, hardware detection, TOML loading
    ...
    cli/                CLI commands (Click-based)
        ask.py              DEMON ask -- query the assistant
        serve.py            DEMON serve -- start API server

    sdk.py              DEMON class -- high-level Python SDK
