...
DEMON orchestrator agent.

## What This Demonstrates

- **Channel integration** — connecting DEMON to messaging platforms (Slack, WhatsApp, etc.)
...
- Python 3.10+
- DEMON installed: `uv sync --extra dev`
...
2. Configure the WhatsApp Baileys bridge (see the DEMON channel docs).
...
DEMON supports many channels — LINE, Viber, Mastodon, Rocket.Chat, and
...
Refer to `configs/DEMON/config.toml` for the full list of channel and agent
options.
...
this via the DEMON scheduler:

```bash
DEMON scheduler create "Daily inbox summary" --type cron --value "0 17 * * *"
```

Or use the operator recipe pattern to run a persistent triage agent on a
schedule. See `src/DEMON/recipes/data/operators/` for examples.
