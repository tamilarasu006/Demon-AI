...
use DEMON_core::config::DEMONConfig;
...
pub fn discover_engines(config: &DEMONConfig) -> Vec<EngineInfo> {
...
pub fn get_engine_static(
...
) -> Result<crate::engine_enum::Engine, DEMONError> {
    ...
    match key.as_str() {
        ...
        other => Err(DEMONError::Engine(
            DEMON_core::error::EngineError::ModelNotFound(format!(
...
mod tests {
    ...
    fn test_get_engine_static_ollama() {
        let config = DEMONConfig::default();
