...
``~/.DEMON/connectors`` (a real source of spurious failures — see the
...
def hermetic_connectors(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    ...
    *source* (``DEMON.core.config``) — every connector re-derives
    ...
    google_mods = [
        "DEMON.connectors.gdrive",
        "DEMON.connectors.gcalendar",
        "DEMON.connectors.gcontacts",
        "DEMON.connectors.gmail",
        "DEMON.connectors.google_tasks",
    ]
    ...
    # subsequent tests in the same process see ~/.DEMON again.
    for name in google_mods:
        if name in sys.modules:
            importlib.reload(sys.modules[name])
...
def test_connect_client_pair_returns_oauth_required_no_browser(
    ...
    """
    with patch("DEMON.core.open_browser") as mock_browser:
        resp = client.post(
