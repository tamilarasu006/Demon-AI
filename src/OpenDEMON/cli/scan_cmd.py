...
class PrivacyScanner:
    ...
    def check_icloud_sync(self) -> ScanResult:
        """Check whether ~/.DEMON is inside iCloud Drive sync scope."""
        try:
            ...
            if str(config_path).startswith(str(icloud_path)):
                return ScanResult(
                    name="iCloud Sync",
                    status="warn",
                    message="~/.DEMON may be synced to iCloud.",
                    platform="darwin",
                )
            ...
            return ScanResult(
                name="iCloud Sync",
                status="ok",
                message="~/.DEMON is not inside iCloud Drive.",
                platform="darwin",
            )
        except Exception:
            return ScanResult(
...
def _render_results(results: List[ScanResult]) -> None:
    ...
    console = Console()
    console.print()
    console.print("[bold]DEMON Security Scan[/bold]")
    console.print()
