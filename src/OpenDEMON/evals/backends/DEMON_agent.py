...
class DEMONAgentBackend(InferenceBackend):
    """Agent-level inference via SystemBuilder + DEMONSystem.ask().

    Supports tool calling via the agent harness. Works for both local
    and cloud models.
    """

    backend_id = "DEMON-agent"
    framework_name = "DEMON"
    ...
    def __init__(
        ...
        # Override the agent's per-run turn budget. DEMONConfig.agent.max_turns
