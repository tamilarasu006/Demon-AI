/**
 * deep-link.ts — Handler for DEMON:// deep links in the Tauri desktop app.
 *
 ...
 *   DEMON://research/{session_id}   → open a research session
 *   DEMON://connector/{connector_id} → open a connector settings panel
 */
...
/**
 * Parse an `DEMON://` deep link URL into a structured target.
 *
 * @param url - The raw deep link URL string (e.g. `DEMON://research/abc123`).
 * @returns A {@link DeepLinkTarget} if the URL is valid, or `null` if it
 *   cannot be parsed or does not use the `DEMON:` scheme.
 *
 * @example
 * parseDeepLink("DEMON://research/abc123");
 * // → { type: "research", id: "abc123" }
 *
 * parseDeepLink("DEMON://connector/gmail");
...
export function parseDeepLink(url: string): DeepLinkTarget | null {
  try {
    ...
    if (parsed.protocol !== "DEMON:") return null;

    // The URL constructor treats "DEMON://research/abc123" such that
