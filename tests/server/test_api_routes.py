...
class TestMemoryRustMissing:
    """Regression for #502: when the native ``DEMON_rust`` extension is
    ...
    def _client(monkeypatch):
        ...
        def _boom():
            raise ImportError("No module named 'DEMON_rust'")

        import OpenDEMON._rust_bridge as bridge
    ...
    def test_config_reports_unavailable(self, monkeypatch):
        ...
        assert "DEMON_rust" in (data["detail"] or "")
...
class TestMetricsRoute:
    def test_metrics_endpoint(self):
        ...
        assert resp.status_code == 200
        assert "DEMON" in resp.text or "No metrics" in resp.text
