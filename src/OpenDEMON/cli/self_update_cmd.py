...
- PyPI installs get ``pip install --upgrade DEMON``.
- uv-tool installs get ``uv tool upgrade DEMON``.
...
@click.command(
    "self-update",
    help=(
        "Upgrade DEMON to the latest release. Detects how you "
...
def self_update(check: bool, yes: bool) -> None:
    ...
    click.echo("\nUpgrade complete. Re-run `DEMON --version` to confirm.")
