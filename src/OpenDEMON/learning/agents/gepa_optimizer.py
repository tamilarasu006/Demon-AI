...
class DEMONGEPAAdapter:
    """Implements GEPA's adapter protocol for DEMON agents.
    ...
    def optimize(self, trace_store: Any) -> Dict[str, Any]:
        ...
        if not HAS_GEPA:
            return {
                "status": "error",
                "reason": (
                    "gepa not installed (pip install 'DEMON[learning-gepa]')"
                ),
            }

        agent_name = self.config.agent_filter or "default"
        adapter = DEMONGEPAAdapter(trace_store, agent_name, self.config)
        adapter.load_traces()
...
__all__ = ["GEPAAgentOptimizer", "DEMONGEPAAdapter"]
