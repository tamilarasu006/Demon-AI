...
def _ensure_identity_prompt(messages: list[Message], app_config) -> list[Message]:
    """Prepend DEMON's identity system prompt when the client omits one.
    ...
    *,
    ...
    async def generate():
        ...
        try:
            ...
            logging.getLogger("DEMON.server").error(
                "Tool stream error: %s",
                exc,
                exc_info=True,
            )
...
async def reload_cloud_engine(request: Request):
    ...
    # Re-read ~/.DEMON/cloud-keys.env and update the running process env.
