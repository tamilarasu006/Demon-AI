#!/usr/bin/env bash
# install.sh — DEMON curl-pipe-bash installer.
#
# Usage:
...
#   DEMON_HOME        Install dir (default: $HOME/.DEMON)
#   DEMON_REPO_URL    git repo URL (default: https://github.com/DEMON/DEMON.git)
#   DEMON_FORCE_WSL   Set 1 to force WSL detection (testing)
...
# uv/git tooling installs to Windows paths the rest of DEMON can't
...
DEMON runs on Windows via WSL2. Two paths:
  ...
  2. Desktop app — download the .exe from the Releases page:
     https://github.com/DEMON/DEMON/releases

See the WSL2 install guide for the full walkthrough:
  https://DEMON.github.io/DEMON/getting-started/wsl2/
EOF
        exit 1
        ;;
esac
...
DEMON installs to $HOME/.DEMON, not /usr/local. Re-run as your
...
Two ways forward:
  ...
  2. Pre-authenticate sudo before piping (caches credentials for 5 min):
       sudo -v && curl -fsSL https://DEMON.github.io/DEMON/install.sh | bash
EOF
        exit 1
    fi
    local sudo=""
...
# ---- env ----
# DEMON keeps ALL of its state (install tree + runtime data, configs,
# databases, caches, logs) under a single root so it never clutters $HOME
# beyond one directory. Relocate it by exporting DEMON_HOME before
# running the installer, e.g.:
#     DEMON_HOME=~/apps/DEMON curl ... | bash
# The Python runtime honors the same override (and, when DEMON_HOME is
# unset, $XDG_DATA_HOME/DEMON if XDG_DATA_HOME is set). With nothing set
# the root is ~/.DEMON, so existing installs are untouched.
DEMON_HOME="${DEMON_HOME:-$HOME/.DEMON}"
DEMON_REPO_URL="${DEMON_REPO_URL:-https://github.com/DEMON/DEMON.git}"
SRC_DIR="$DEMON_HOME/src"
VENV_DIR="$DEMON_HOME/.venv"
STATE_DIR="$DEMON_HOME/.state"
SCRIPTS_DIR="$DEMON_HOME/.scripts"
STATE_FILE="$STATE_DIR/install-state.json"

mkdir -p "$DEMON_HOME" "$STATE_DIR" "$SCRIPTS_DIR"

# ---- WSL detection ----
WSL=0
if [[ "${DEMON_FORCE_WSL:-0}" == "1" ]]; then
    WSL=1
...
# DEMON team can see where users drop off during install.
...
#
ANALYTICS_HOST="${DEMON_ANALYTICS_HOST:-https://34.231.106.201.sslip.io}"
ANALYTICS_KEY="${DEMON_ANALYTICS_KEY:-phc_ysKu72QaxzYNmDpHFcesD2ZZAe68zkdWJEKoYYkc5e3n}"
ANON_ID_FILE="$DEMON_HOME/anon_id"
...
clone_repo() {
    ...
    git clone --depth 1 "$DEMON_REPO_URL" "$SRC_DIR"
}
...
write_config() {
    "$VENV_DIR/bin/DEMON" _bootstrap --write-config \
...
install_symlinks() {
    ...
    ln -sf "$SCRIPTS_DIR/DEMON-wrapper.sh" "$HOME/.local/bin/DEMON"
    ln -sf "$SCRIPTS_DIR/DEMON-uninstall.sh" "$HOME/.local/bin/DEMON-uninstall"
}

# Tracks whether the user needs to source ~/.bashrc / ~/.zshrc / open a
# new terminal before `DEMON` will resolve. Set only when ensure_path
...
ensure_path() {
    ...
    if grep -q "DEMON" "$rc" 2>/dev/null; then
...
from DEMON.core.config import detect_hardware, recommend_model
hw = detect_hardware()
...
# ---- run ----
echo "DEMON installer"
echo "  install dir: $DEMON_HOME"
echo "  WSL2:        $WSL"
echo
...
step clone_repo         "Clone DEMON repo" clone_repo
step copy_scripts       "Copy install scripts"  copy_scripts
step create_venv        "Create venv"           create_venv
step editable_install   "Install DEMON"    editable_install
...
#
#   PATH ok + model ok   -> "type DEMON"
#   PATH new + model ok  -> "source rc && DEMON  (or open new terminal)"
#   PATH ok + model bad  -> "model still downloading; DEMON doctor"
#   PATH new + model bad -> "source rc && DEMON doctor; chat works once download finishes"
#
# `DEMON` and `DEMON doctor` need PATH equally, so the source/restart
# guidance goes first when PATH was modified.
NEXT_CMD="DEMON"
if [[ "$MODEL_PULL_OK" -ne 1 ]]; then
    NEXT_CMD="DEMON doctor"
fi
...
if [[ "$MODEL_PULL_OK" -ne 1 ]]; then
    cat <<EOF
NOTE: the qwen3.5:2b model didn't finish downloading. 'DEMON doctor'
...
Background work continues silently:
  ...
  Run 'DEMON doctor' to check status anytime.
EOF
