...
5. Clones the DEMON repository to `%LOCALAPPDATA%\DEMON`
...
The available env vars: `DEMON_SKIP_SERVICE`, `DEMON_SERVICE`,
`DEMON_FORCE`. If you need richer control, save the script first
...
remove the task with `DEMON-service.ps1`:

```powershell
$srv = "$env:LOCALAPPDATA\DEMON\src\deploy\windows\DEMON-service.ps1"
...
`DEMON-service.ps1 install` refuses `-ListenHost 0.0.0.0` if
`$env:DEMON_API_KEY` is unset — same guard as the systemd unit's
`EnvironmentFile=/etc/DEMON/env`.

## Parity table

| Concern | systemd | launchd | Windows |
|---------|---------|---------|---------|
| Service definition | `deploy/systemd/DEMON.service` | `deploy/launchd/com.DEMON.plist` | `deploy/windows/DEMON-service.ps1` (cmdlet-driven) |
...
```powershell
powershell -ExecutionPolicy Bypass -File "$env:LOCALAPPDATA\DEMON\src\deploy\windows\DEMON-service.ps1" uninstall
Remove-Item -Recurse -Force "$env:LOCALAPPDATA\DEMON"
```
