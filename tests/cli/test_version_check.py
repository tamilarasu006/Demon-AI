...
def _clean_env(monkeypatch, tmp_path):
    for v in ("DEMON_NO_UPDATE_CHECK", "CI", "DEMON_CONFIG"):
        monkeypatch.delenv(v, raising=False)
    # Point config + cache at empty tmp paths so tests don't see the
    # developer's real ~/.DEMON state.
    monkeypatch.setenv("DEMON_CONFIG", str(tmp_path / "no-config.toml"))
...
class TestCheckDisabled:
    ...
    def test_DEMON_no_update_check_disables(self, monkeypatch, value):
        monkeypatch.setenv("DEMON_NO_UPDATE_CHECK", value)
...
class TestConfigDisabled:
    ...
    def test_auto_update_false_disables(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setenv("DEMON_CONFIG", str(cfg))
    ...
    def test_DEMON_config_env_override(self, monkeypatch, tmp_path):
        """DEMON_CONFIG should redirect the lookup, matching core.config."""
...
class TestCheckForUpdates:
    @patch("DEMON.cli._version_check._do_check")
    ...
    @patch(
        "DEMON.cli._version_check._do_check",
