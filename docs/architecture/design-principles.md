...
DEMON follows a set of design principles that guide every architectural decision. These principles ensure the framework remains extensible, portable, and easy to work with.

---

## 1. Pluggable Everything

Every major component in DEMON is defined as an **abstract base class** (ABC) with concrete implementations registered at runtime. This means you can swap, extend, or replace any part of the system without modifying existing code.
...
DEMON is designed to work **entirely without network access**. All core functionality -- inference, memory, agents, tools, telemetry -- operates locally. Cloud APIs are optional extensions, never requirements.
...
j = DEMON(engine_key="ollama")  # Local Ollama server
...
DEMON **auto-detects system hardware** at startup and recommends the optimal inference engine. The `detect_hardware()` function probes:
...
This recommendation is written to `config.toml` during `DEMON init` and used as the default engine:

```bash
DEMON init --force
# Detects hardware, writes ~/.DEMON/config.toml with:
# [engine]
...
```bash
DEMON telemetry stats          # Aggregated statistics
DEMON telemetry export --json  # Export all records
```
...
DEMON provides a **clean Python API** through the `DEMON` class. There is no framework lock-in -- the SDK is a standard Python package with dataclass-based types and no required web framework.
...
- **Optional dependencies** via extras (`DEMON[server]`, `DEMON[memory-colbert]`, etc.)
...
The API server (`DEMON serve`) implements the **OpenAI chat completions API format**, making DEMON a drop-in replacement for OpenAI in existing applications.
...
Any OpenAI client library can connect to DEMON:
...
DEMON requires **no external services** for core functionality. Everything needed to run the system is included or uses standard system libraries.
...
The only external requirement is a running inference engine (Ollama, vLLM, etc.), which is the model server itself -- not a dependency of DEMON.

Optional features that require additional packages:

| Feature | Extra | Packages |
|---------|-------|----------|
| FAISS memory | `DEMON[memory-faiss]` | `faiss-cpu`, `sentence-transformers` |
| ColBERT memory | `DEMON[memory-colbert]` | `colbert-ai`, `torch` |
| BM25 memory | `DEMON[memory-bm25]` | `rank-bm25` |
| API server | `DEMON[server]` | `fastapi`, `uvicorn` |
| Cloud inference | `DEMON[inference-cloud]` | `openai`, `anthropic`, `google-genai` |
| vLLM engine | `DEMON[inference-vllm]` | `vllm` |
| PDF ingestion | `DEMON[memory-pdf]` | `pdfplumber` |
| WhatsApp Baileys | `DEMON[channel-whatsapp-baileys]` | Node.js 22+ |
