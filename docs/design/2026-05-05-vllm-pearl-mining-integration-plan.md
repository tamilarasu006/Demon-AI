...
**Goal:** Implement Spec A — the v1 vLLM-Pearl mining integration — as a new `DEMON.mining` subsystem that lets H100/H200 owners running vLLM solo-mine the Pearl PoUW chain with their normal LLM inference.

**Architecture:** New top-level subsystem `mining/` mirrors OJ's existing primitive pattern (peer to `engine/`, `agents/`). A `MiningProvider` ABC + `MinerRegistry` enables future hardware/engine paths (Apple Silicon, AMD, Ollama) without rewrite. The v1 `vllm-pearl` provider orchestrates Pearl's published Docker container; a runtime sidecar at `~/.DEMON/runtime/mining.json` decouples mining lifecycle from the existing vLLM engine class. Three deliberate seams (`submit_target` tagged-union, zero-valued `fee_bps`/`fees_owed`, reserved `mining/pools/`) leave room for v2 pool support without retrofit pain.
...
- Absolute imports only (`from DEMON.core.registry import ...`).
...
| Path | Responsibility |
|---|---|
| `src/DEMON/mining/__init__.py` | Package init; soft-imports providers via `try/except ImportError` |
| `src/DEMON/mining/_stubs.py` | `MiningProvider` ABC, `MiningCapabilities`, `MiningConfig`, `MiningStats`, `SoloTarget`/`PoolTarget` tagged union, `Sidecar` dataclass + read/write helpers |
| `src/DEMON/mining/_discovery.py` | `detect_providers()`, hardware/Docker/disk/pearld checks, wallet-format check |
| `src/DEMON/mining/_constants.py` | `PEARL_REPO`, `PEARL_PINNED_REF`, `PEARL_IMAGE_TAG`, default ports, default model, sidecar path constants |
| `src/DEMON/mining/_docker.py` | `PearlDockerLauncher` — `ensure_image()`, `start()`, `stop()`, `is_running()`, `get_logs()` |
| `src/DEMON/mining/_collector.py` | `MiningTelemetryCollector` — full impl, shipped unwired in v1; v1.x lights it up in the gateway daemon |
| `src/DEMON/mining/vllm_pearl.py` | `VllmPearlProvider` (`MiningProvider` impl); `_parse_gateway_metrics()`; `ensure_registered()` |
| `src/DEMON/mining/pools/__init__.py` | Reserved location for v2 `PoolClient` work — empty except for a docstring saying so |
| `src/DEMON/cli/mine_cmd.py` | `DEMON mine` Click group: `init`, `start`, `stop`, `status`, `doctor`, `attach`, `logs` |
...
| Path | Change |
|---|---|
| `src/DEMON/core/registry.py` | Add `MinerRegistry` class + entry in `__all__` |
| `src/DEMON/core/config.py` | Add `MiningConfig` field to `DEMONConfig`; add TOML parser for `[mining]` with `submit_target` tagged-union resolution |
| `src/DEMON/engine/_discovery.py` | Add sidecar-aware engine resolution: when `~/.DEMON/runtime/mining.json` exists, register a derived `vllm` engine pointing at `vllm_endpoint` |
| `src/DEMON/telemetry/store.py` | Add nullable `mining_session_id` column to inference rows; bump `PRAGMA user_version`; helper to tag rows when sidecar is present |
| `src/DEMON/cli/__init__.py` | Register the `mine` Click group |
| `src/DEMON/cli/hints.py` | Add hint: "mining configured but not running — start it with `DEMON mine start`" |
...
**Files:**
- Modify: `src/DEMON/core/registry.py`
...
class MinerRegistry(RegistryBase[Any]):
    ...
    ``DEMON.mining._stubs``. Registry keys are short lowercase strings
...
```bash
git add src/DEMON/core/registry.py tests/conftest.py tests/core/test_registry.py
...
**Files:**
- Create: `src/DEMON/mining/__init__.py`
- Create: `src/DEMON/mining/_constants.py`
- Create: `src/DEMON/mining/_stubs.py`
- Create: `src/DEMON/mining/pools/__init__.py`
...
def test_mining_provider_is_abstract():
    from DEMON.mining._stubs import MiningProvider
...
```
Expected: ALL fail with `ImportError` on `DEMON.mining._stubs`.
...
PEARL_IMAGE_TAG = f"DEMON/pearl-miner:{PEARL_PINNED_REF}"
...
RUNTIME_DIR = Path.home() / ".DEMON" / "runtime"
...
PEARL_CACHE_DIR = Path.home() / ".DEMON" / "cache" / "pearl"
```
...
class MiningCapabilities:
    ...
    ``reason`` is human-readable and surfaced verbatim by ``DEMON mine doctor``
...
class MiningProvider(ABC):
    ...
    def detect(cls, hw: HardwareInfo, engine_id: str, model: str) -> MiningCapabilities:
        ...
        by ``DEMON mine doctor`` and ``DEMON mine init`` for fast capability
        reporting.
        """
...
class Sidecar:
    """Read/write helpers for ``~/.DEMON/runtime/mining.json``."""
...
```python
# src/DEMON/mining/pools/__init__.py
...
```bash
git add src/DEMON/mining/ tests/mining/__init__.py tests/mining/conftest.py tests/mining/test_stubs.py
git commit -m "feat(mining): add ABC, dataclasses, constants, sidecar IO"
```

---

## Task 3 — `MiningConfig` integration in `DEMONConfig` + TOML parsing

**Files:**
- Modify: `src/DEMON/core/config.py`
...
provider       = "vllm-pearl"
wallet_address = "prl1qexampleaddress"
submit_target  = "pool:https://pool.DEMON.ai/submit"
...
def test_mining_config_pool_parsed_as_pool_target(tmp_path):
    ...
    assert cfg.mining.submit_target.url == "https://pool.DEMON.ai/submit"
```
...
- [ ] **Step 4: Add `mining` field to `DEMONConfig`**

In `src/DEMON/core/config.py`, add an import near the top:
...
Add the field to `DEMONConfig` (insert in alphabetical order — after `memory_files`, before `operators`):
...
In `src/DEMON/core/config.py`, locate the existing `load_config()` function (around line 1541). Find the section that processes the loaded TOML data into the `DEMONConfig` (look for where it iterates the `data` dict and assigns to dataclass fields). Add a dedicated handler for `mining` because of the tagged-union parsing — it can't go through the generic walker.
...
```bash
git add src/DEMON/core/config.py tests/core/test_config.py tests/mining/fixtures/
git commit -m "feat(mining): integrate MiningConfig into DEMONConfig with TOML parsing"
```
...
**Files:**
- Create: `src/DEMON/mining/_discovery.py`
...
def detect_for_engine_model(
    *,
    ...
    ``DEMON mine doctor`` and ``DEMON mine init``.
    """
...
# ---------------------------------------------------------------------------
# Doctor checks (one per row of `DEMON mine doctor` output)
# ---------------------------------------------------------------------------
...
```bash
git add src/DEMON/mining/_discovery.py tests/mining/test_discovery.py
...
**Files:**
- Create: `src/DEMON/mining/_docker.py`
...
def test_ensure_image_already_local():
    ...
    fake = MagicMock()
    fake.images.get.return_value = MagicMock(id="sha256:abc", tags=["DEMON/pearl-miner:main"])
    launcher = PearlDockerLauncher(client=fake)
    out = launcher.ensure_image("DEMON/pearl-miner:main")
    assert out == "DEMON/pearl-miner:main"
    fake.images.get.assert_called_once_with("DEMON/pearl-miner:main")
...
def test_ensure_image_errors_when_non_default_tag_missing():
    from DEMON.mining._docker import PearlDockerLauncher, ImageAcquisitionError
    import docker.errors as derr
    import pytest
    fake = MagicMock()
    fake.images.get.side_effect = derr.ImageNotFound("nope")
...
```bash
git add src/DEMON/mining/_docker.py tests/mining/test_docker.py
...
**Files:**
- Modify: `src/DEMON/mining/_docker.py`
...
def test_launcher_start_calls_run_with_expected_kwargs(_env_password):
    ...
    cfg = MiningConfig(
        ...
        submit_target=SoloTarget(pearld_rpc_url="http://localhost:44107"),
        extra={
            "docker_image_tag": "DEMON/pearl-miner:main",
            "model": "pearl-ai/Llama-3.3-70B-Instruct-pearl",
            "vllm_port": 8000,
...
def test_launcher_start_errors_when_password_env_missing():
    from DEMON.mining._docker import PearlDockerLauncher, ConfigurationError
    from DEMON.mining._stubs import MiningConfig, SoloTarget
    fake = MagicMock()
    launcher = PearlDockerLauncher(client=fake)
    cfg = MiningConfig(
    ...
    with pytest.raises(ConfigurationError) as ei:
        launcher.start(cfg, image="DEMON/pearl-miner:main")
...
Append to `src/DEMON/mining/_docker.py`:
...
class ConfigurationError(RuntimeError):
    ...
    def start(self, config: "MiningConfig", image: str) -> Any:
        ...
        """
        from DEMON.mining._stubs import MiningConfig  # noqa: F401  (typing only)
        ...
        if password is None:
            raise ConfigurationError(
                ...
                f"set it before running `DEMON mine start`"
            )
...
**Files:**
- Modify: `src/DEMON/mining/vllm_pearl.py` (creating in next task — for now create the parser as a helper)
- Create: `src/DEMON/mining/_metrics.py`
...
```bash
git add src/DEMON/mining/_metrics.py tests/mining/test_metrics.py tests/mining/fixtures/
...
**Files:**
- Create: `src/DEMON/mining/vllm_pearl.py`
...
async def test_vllm_pearl_start_writes_sidecar(tmp_path, monkeypatch):
    from DEMON.mining.vllm_pearl import VllmPearlProvider
    from DEMON.mining._stubs import MiningConfig, SoloTarget, Sidecar

    sidecar_path = tmp_path / "mining.json"
    monkeypatch.setattr(
        "DEMON.mining.vllm_pearl.SIDECAR_PATH", sidecar_path
    )
...
async def test_vllm_pearl_start_pool_target_raises_not_implemented(monkeypatch, tmp_path):
    ...
    cfg = MiningConfig(
        provider="vllm-pearl",
        wallet_address="prl1qaaa",
        submit_target=PoolTarget(url="https://pool.DEMON.ai/submit"),
        extra={"docker_image_tag": "DEMON/pearl-miner:main"},
    )
...
async def test_vllm_pearl_stop_removes_sidecar(tmp_path, monkeypatch, written_sidecar):
    from DEMON.mining.vllm_pearl import VllmPearlProvider
    monkeypatch.setattr(
        "DEMON.mining.vllm_pearl.SIDECAR_PATH", written_sidecar
    )
    fake_client = MagicMock()
    provider = VllmPearlProvider(docker_client=fake_client)
...
def test_vllm_pearl_stats_reads_gateway(monkeypatch, written_sidecar):
    ...
    with patch("DEMON.mining.vllm_pearl.httpx.get") as get:
    ...
    async def start(self, config: MiningConfig) -> None:
        if isinstance(config.submit_target, PoolTarget):
            raise NotImplementedError(
                "pool support is v2 — see DEMON#XYZ. v1 only accepts "
                "submit_target='solo'."
            )
...
```bash
git add src/DEMON/mining/vllm_pearl.py tests/mining/test_vllm_pearl.py
...
**Files:**
- Modify: `src/DEMON/engine/_discovery.py`
...
Run:
```bash
sed -n '1,80p' src/DEMON/engine/_discovery.py
```
...
def test_engine_discovery_picks_up_mining_sidecar(tmp_path, monkeypatch, written_sidecar):
    ...
    """
    from DEMON.engine._discovery import discover_engines
...
def _maybe_register_mining_sidecar_engine() -> None:
    ...
    from DEMON.engine.openai_compat_engines import OpenAICompatEngine  # adjust to actual class name
...
```bash
git add src/DEMON/engine/_discovery.py tests/conftest.py tests/mining/conftest.py tests/engine/test_discovery.py
...
**Files:**
- Create: `src/DEMON/mining/_collector.py`
...
this as a periodic asyncio task in ``DEMON.daemon.gateway``. v1's
...
```bash
git add src/DEMON/mining/_collector.py tests/mining/test_collector.py
...
**Files:**
- Modify: `src/DEMON/telemetry/store.py`
...
Run:
```bash
grep -n "PRAGMA user_version\|CREATE TABLE\|ALTER TABLE\|migrate" src/DEMON/telemetry/store.py | head -20
```
...
In `src/DEMON/telemetry/store.py`:
...
```bash
git add src/DEMON/telemetry/store.py tests/telemetry/test_store.py
git commit -m "feat(telemetry): add mining_session_id + mining_stats table"
```

---

## Task 12 — `DEMON mine doctor` (highest user-facing value, build first)

**Files:**
- Create: `src/DEMON/cli/mine_cmd.py`
...
def test_mine_doctor_prints_capability_matrix(monkeypatch):
    ...
    with patch("DEMON.cli.mine_cmd._detect_hardware", return_value=fake_hw), \
         patch("DEMON.cli.mine_cmd.check_docker_available", return_value=(True, "running 24.0.7")), \
         patch("DEMON.cli.mine_cmd.check_disk_free", return_value=(True, "300 GB free")), \
         patch("DEMON.cli.mine_cmd.check_pearld_reachable", return_value=(True, "block height 442107 (synced)")):
...
def test_mine_doctor_flags_unsupported_hardware():
    ...
    with patch("DEMON.cli.mine_cmd._detect_hardware", return_value=fake_hw), \
         patch("DEMON.cli.mine_cmd.check_docker_available", return_value=(True, "ok")), \
         patch("DEMON.cli.mine_cmd.check_disk_free", return_value=(True, "300 GB free")), \
         patch("DEMON.cli.mine_cmd.check_pearld_reachable", return_value=(False, "connection refused")):
        result = runner.invoke(mine, ["doctor"])
...
from DEMON.core.config import HardwareInfo, load_config
from DEMON.mining._constants import (
    DEFAULT_PEARL_MODEL,
...
def mine() -> None:
    ...
    See https://DEMON.github.io/DEMON/user-guide/mining/ for the
    full guide.
    """
...
def doctor() -> None:
    ...
    if mining_cfg is not None:
        ...
        row("pearld", "RPC", False, "no [mining] config — run `DEMON mine init`")
...
```bash
git add src/DEMON/cli/mine_cmd.py tests/mining/test_cli.py
git commit -m "feat(mining-cli): DEMON mine doctor"
```

---

## Task 13 — `DEMON mine init / start / stop`

**Files:**
- Modify: `src/DEMON/cli/mine_cmd.py`
...
def test_mine_start_runs_provider_start(monkeypatch):
    ...
    with patch("DEMON.cli.mine_cmd.MinerRegistry") as reg, \
         patch("DEMON.cli.mine_cmd.load_config") as load, \
         patch("DEMON.cli.mine_cmd.asyncio.run") as arun:
        from DEMON.mining._stubs import MiningConfig, SoloTarget
...
def init(
    ...
    if not cap.supported:
        raise click.ClickException(
            ...
            f"See `DEMON mine doctor` for details."
        )
    ...
    if pearld_password_env not in os.environ:
        click.echo(
            ...
            f"Set it before `DEMON mine start`.",
            err=True,
        )
    ...
    config_path = Path.home() / ".DEMON" / "config.toml"
...
"""
    ...
    click.echo(f"Done. Run `DEMON mine start` to begin mining.")
...
def start() -> None:
    ...
    if cfg is None:
        raise click.ClickException("no [mining] section in config — run `DEMON mine init`")
    ...
    async def _run():
        await provider.start(cfg)
    asyncio.run(_run())
    click.echo(f"Mining started. Run `DEMON mine status` for live stats.")
...
```bash
git add src/DEMON/cli/mine_cmd.py tests/mining/test_cli.py
git commit -m "feat(mining-cli): DEMON mine init/start/stop"
```

---

## Task 14 — `DEMON mine status / attach / logs`
...
def test_mine_status_renders_stats(written_sidecar, monkeypatch):
    from DEMON.cli.mine_cmd import mine
    runner = CliRunner()
    monkeypatch.setattr("DEMON.cli.mine_cmd.SIDECAR_PATH", written_sidecar)
    sample = (
        "pearl_gateway_shares_submitted_total 100\n"
        "pearl_gateway_shares_accepted_total 99\n"
    ...
    with patch("DEMON.mining.vllm_pearl.httpx.get") as get, \
         patch("DEMON.cli.mine_cmd.MinerRegistry") as reg:
        get.return_value.status_code = 200
        get.return_value.text = sample
        from DEMON.mining.vllm_pearl import VllmPearlProvider
...
def test_mine_attach_writes_sidecar(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("DEMON.cli.mine_cmd.SIDECAR_PATH", sidecar)
...
def test_mine_logs_streams_container_output(monkeypatch):
    ...
    with patch("DEMON.cli.mine_cmd.PearlDockerLauncher",
...
def status() -> None:
    ...
    cfg = load_config().mining
    if cfg is None:
        raise click.ClickException("no [mining] section — run `DEMON mine init`")
    provider_cls = MinerRegistry.get(cfg.provider)
    provider = provider_cls()
    s = provider.stats()
...
def logs(tail_n: int, follow: bool) -> None:
    ...
    try:
        container = docker.from_env().containers.get("DEMON-pearl-miner")
...
```bash
git add src/DEMON/cli/mine_cmd.py tests/mining/test_cli.py
git commit -m "feat(mining-cli): DEMON mine status/attach/logs"
```
...
**Files:**
- Modify: `src/DEMON/cli/__init__.py`
- Modify: `src/DEMON/cli/hints.py`
...
```bash
sed -n '1,50p' src/DEMON/cli/__init__.py
grep -n "add_command\|@main.command\|main.add_command" src/DEMON/cli/__init__.py | head -20
```
...
def mining_not_running_hint(cfg, sidecar_present: bool) -> Optional[str]:
    if cfg is None or sidecar_present:
        return None
    return "mining configured but not running — start it with `DEMON mine start`"
```
...
```bash
git add src/DEMON/cli/__init__.py src/DEMON/cli/hints.py tests/cli/test_main.py tests/cli/test_hints.py
...
DEMON can mine the [Pearl](https://github.com/pearl-research-labs/pearl)
...
uv run DEMON mine init    # writes [mining] config + builds Docker image (30-60 min first time)
uv run DEMON mine start
uv run DEMON mine status
```

## Diagnosing problems

`DEMON mine doctor` prints one row per check with a clear ✓ or ✗ and reason.
...
The `DEMON.mining` subsystem uses a registry pattern identical to
...
1. Create `src/DEMON/mining/<provider>.py`.
2. Subclass `DEMON.mining.MiningProvider`.
...
5. Add a soft-import in `mining/__init__.py`:
   ```python
   try:
       from DEMON.mining import <provider>  # noqa: F401
...
```markdown
- New mining providers must register via `MinerRegistry` in `src/DEMON/core/registry.py` and expose an idempotent `ensure_registered()` per the autouse-clear test convention.
```
...
```bash
uv run pytest tests/mining/ --cov=DEMON.mining --cov-report=term-missing
```
Expected: ≥ 80% coverage on the new `mining/` package.

- [ ] **Step 4: Smoke `DEMON mine doctor` on a non-mining dev box**

```bash
uv run DEMON mine doctor
```
...
## Summary
- New `DEMON.mining` subsystem with `MiningProvider` ABC + `MinerRegistry`
- `vllm-pearl` provider wrapping Pearl's Docker container
- Runtime sidecar (`~/.DEMON/runtime/mining.json`) for engine ↔ mining handoff
- Telemetry: on-demand reads in v1; `MiningTelemetryCollector` shipped unwired for v1.x
- New CLI: `DEMON mine init|start|stop|status|doctor|attach|logs`
...
- [ ] Manual smoke: `uv run DEMON mine doctor` on a non-mining dev box prints honest output
