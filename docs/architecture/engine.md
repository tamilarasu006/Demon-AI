...
The Engine primitive provides the **inference runtime** -- the layer that connects DEMON to language model servers. All backends implement a uniform interface, making it straightforward to swap between local and cloud inference without changing application code.
...
DEMON automatically detects system hardware to recommend the best engine. Detection runs at config load time via `detect_hardware()`:
...
```python
from DEMON.engine import discover_engines
...
Engine hosts and defaults are configured in `~/.DEMON/config.toml` using **nested per-engine sub-sections**:
...
```python
from DEMON.engine import EngineConnectionError
