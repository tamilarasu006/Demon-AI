...
The DEMON desktop app ships with [Tauri's updater
...
on launch / every 30 min
        │
        ▼
GET https://github.com/DEMON/DEMON/releases/download/desktop-latest/latest.json
        │
        ▼
...
For frontend development, set `VITE_DEMON_NO_UPDATER=1` in your
...
```bash
export VITE_DEMON_NO_UPDATER=1
npm run tauri dev
```

This is purely a dev escape hatch — it has no effect on production
builds (where `import.meta.env.VITE_DEMON_NO_UPDATER` will be
...
curl -fsSL https://github.com/DEMON/DEMON/releases/download/desktop-latest/latest.json | jq .
