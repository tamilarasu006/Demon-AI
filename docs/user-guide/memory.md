...
- **Persistence:** SQLite database file (default: `~/.DEMON/memory.db`)
...
```python
from DEMON.tools.storage.bm25 import BM25Memory
...
!!! note "Backward compatibility"
    The old `from DEMON.memory.bm25 import BM25Memory` still works via backward-compatibility shims, but new code should use the canonical `DEMON.tools.storage.*` imports.
...
[Source: docs/intro.md] DEMON is a modular AI framework...
...
DEMON memory index ./notes/ --chunk-size 256 --chunk-overlap 32

# Search the memory store
DEMON memory search "machine learning"
