...
def _make_system(engine=None, agent_name="", tmp_path=None) -> DEMONSystem:
    """Build a minimal DEMONSystem with mock engine for testing wire_channel."""
    config = DEMONConfig()
...
# ---------------------------------------------------------------------------
# Tests via DEMONSystem.wire_channel()
# ---------------------------------------------------------------------------
...
class TestChannelToolLoading:
    """DEMONSystem receives tools when agent accepts them."""
    ...
    def test_non_tool_agent_receives_empty_tools(self, tmp_path):
        """DEMONSystem with no tools list results in empty tools — simple agent
        unaffected."""
        config = DEMONConfig()
        config.sessions.db_path = str(tmp_path / "sessions.db")
