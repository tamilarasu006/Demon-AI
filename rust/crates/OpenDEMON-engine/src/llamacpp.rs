...
use DEMON_core::error::{EngineError, DEMONError};
use DEMON_core::{GenerateResult, Message, Usage};
...
impl InferenceEngine for LlamaCppEngine {
    ...
    fn generate(
        ...
        max_tokens: i64,
        _extra: Option<&Value>,
    ) -> Result<GenerateResult, DEMONError> {
        ...
        let resp = self
            ...
            .send()
            .map_err(|e| {
                DEMONError::Engine(EngineError::Connection(format!(
        ...
        if !resp.status().is_success() {
            ...
            return Err(DEMONError::Engine(EngineError::Http(format!(
                "llama.cpp returned {status}: {body}"
            ))));
        }

        let data: Value = resp.json().map_err(|e| {
            DEMONError::Engine(EngineError::Deserialization(e.to_string()))
        })?;
    ...
    async fn stream(
        ...
        let async_client = reqwest::Client::builder()
            ...
            .build()
            .map_err(|e| {
                DEMONError::Engine(EngineError::Connection(e.to_string()))
            })?;
        ...
        let token_stream = byte_stream.filter_map(|chunk_result| async {
            match chunk_result {
                ...
                Err(e) => Some(Err(DEMONError::Engine(EngineError::Streaming(
                    e.to_string(),
                )))),
            }
        });
    ...
    fn list_models(&self) -> Result<Vec<String>, DEMONError> {
        ...
        let resp = self
            ...
            .send()
            .map_err(|_| {
                DEMONError::Engine(EngineError::Connection(
