...
A lightweight conversational AI with no tools and no agent overhead. This is the simplest possible DEMON setup: just Ollama and a local model. Ideal for general-purpose chat, Q&A, brainstorming, and getting started quickly.
...
### 2. Install and initialize DEMON

```bash
git clone https://github.com/DEMON/DEMON.git
cd DEMON
uv sync
DEMON init --preset chat-simple
```

### 3. Ask a question

```bash
DEMON ask "What is quantum computing?"
```
...
```bash
# Single question
DEMON ask "Explain the difference between TCP and UDP"
...
DEMON ask -m qwen3.5:9b "Explain general relativity"

# Adjust temperature (0.0 = deterministic, 1.0 = creative)
DEMON ask -t 0.2 "List the planets in our solar system"

# Output raw JSON
DEMON ask --json "What is 2+2?"
```
...
To switch models, either edit `~/.DEMON/config.toml` or override per-query:

```bash
DEMON ask -m qwen3.5:35b "Write a detailed comparison of REST and GraphQL"
```
...
1. Start the backend: `DEMON serve` (or `./scripts/quickstart.sh`)
2. Download and open the desktop app from the [releases page](https://github.com/DEMON/DEMON/releases)
...
DEMON ask -m deepseek-r1:14b "Hello"
```

**Use an environment variable:**

```bash
DEMON_MODEL=qwen3.5:9b DEMON ask "Hello"
```
...
**Browser app not loading** -- Make sure both the backend (`DEMON serve`) and frontend are running. The `./scripts/quickstart.sh` script starts both automatically.
