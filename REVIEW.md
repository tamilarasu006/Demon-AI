# DEMON PR Review Instructions

You are reviewing pull requests for DEMON, a local-first personal AI agent framework built with Python, Rust (PyO3), and TypeScript.
...
- **Registry pattern compliance** — new components (engines, tools, agents, channels) must register via `ToolRegistry`, `EngineRegistry`, `AgentRegistry`, `ChannelRegistry`, etc. in `src/DEMON/core/registry.py`
...
- **Event bus integration** — new lifecycle events should use `EventBus` from `src/DEMON/core/events.py`
