...
DEMON provides a command-line interface through the `DEMON` command. Built on [Click](https://click.palletsprojects.com/), it offers subcommands for querying models, managing memory, running benchmarks, and serving an OpenAI-compatible API.

## Global Options

```bash
DEMON --version   # Print the DEMON version
DEMON --help      # Show top-level help with all subcommands
```

## `DEMON init`

Detect local hardware (CPU, GPU, RAM) and generate a configuration file at `~/.DEMON/config.toml`.

```bash
DEMON init           # Interactive — refuses to overwrite existing config
DEMON init --force   # Overwrite existing config without prompting
```
...
```bash
DEMON ask "Explain quantum computing"
```

**Agent mode** routes the query through an agent that can use tools and manage multi-turn interactions:

```bash
DEMON ask --agent orchestrator "What is 2+2?"
DEMON ask --agent orchestrator --tools calculator,think "Calculate sqrt(144) + 3^2"
DEMON ask --agent simple "Hello"
```
...
```bash
# Basic query
DEMON ask "What is machine learning?"

# Specify a model
DEMON ask -m qwen3:8b "Summarize this concept"

# Use the orchestrator agent with tools
DEMON ask --agent orchestrator --tools calculator "What is 15% of 340?"

# Get JSON output
DEMON ask --json "Hello"

# Disable memory context injection
DEMON ask --no-context "Tell me about Python"

# Set maximum token generation
DEMON ask --max-tokens 2048 "Write a detailed essay about AI"
```
...
DEMON ask -i screenshot.png "What is shown in this image?"

# Send multiple images (the flag is repeatable)
DEMON ask -i chart-a.png -i chart-b.png "Compare these two charts"

# Capture the current screen and ask about it
DEMON ask --screen "Summarize what's on my screen"
```
...
the `DEMON_NUM_CTX` environment variable (default `16384`):

```bash
DEMON_NUM_CTX=8192 DEMON ask --screen "What's on my screen?"
```

!!! note "Keep vision on-device"
    Images are sensitive. DEMON prints a privacy warning before sending
...
Access Pearl's native node, wallet, and RPC tools from the DEMON CLI.

```bash
DEMON pearl doctor
DEMON pearl node -- <pearld args>
DEMON pearl wallet -- <oyster args>
DEMON pearl ctl -- <prlctl args>
DEMON pearl address
```

All Pearl wrapper commands use the `DEMON pearl <command>` shape. The
pass-through commands map to Pearl's native binaries:

| DEMON command | Pearl binary | Use |
|--------------------|--------------|-----|
| `DEMON pearl doctor` | n/a | Check whether `pearld`, `oyster`, and `prlctl` are discoverable |
| `DEMON pearl node` | `pearld` | Run the Pearl full node |
| `DEMON pearl wallet` | `oyster` | Run the Oyster wallet daemon |
| `DEMON pearl ctl` | `prlctl` | Query Pearl node or wallet RPC |
| `DEMON pearl address` | `prlctl --wallet getnewaddress` | Generate a wallet address from Oyster |
...
### `DEMON memory index <path>`

Index documents from a file or directory into the memory store.

```bash
DEMON memory index ./docs/
DEMON memory index ./notes.md
DEMON memory index ./data/ --chunk-size 256 --chunk-overlap 32
DEMON memory index ./docs/ --backend sqlite
```
...
### `DEMON memory search <query>`

Search the memory store for relevant document chunks.

```bash
DEMON memory search "machine learning basics"
DEMON memory search -k 10 "neural networks"
DEMON memory search --backend faiss "embeddings"
```
...
```bash
DEMON memory stats
DEMON memory stats --backend sqlite
```
...
```bash
DEMON telemetry stats
DEMON telemetry stats -n 5    # Show top 5 models
```
...
### `DEMON telemetry export`

Export raw telemetry records in JSON or CSV format.

```bash
DEMON telemetry export                          # JSON to stdout
DEMON telemetry export --format csv             # CSV to stdout
DEMON telemetry export --format json -o data.json  # JSON to file
DEMON telemetry export -f csv -o metrics.csv    # CSV to file
```
...
### `DEMON telemetry clear`

Delete all telemetry records from the database.

```bash
DEMON telemetry clear         # Interactive confirmation
DEMON telemetry clear --yes   # Skip confirmation
```
...
```bash
DEMON bench run                               # Run all benchmarks, 10 samples
DEMON bench run -n 20                         # 20 samples per benchmark
DEMON bench run -b latency                    # Only the latency benchmark
DEMON bench run -b throughput -n 50 --json    # Throughput, 50 samples, JSON output
DEMON bench run -o results.jsonl              # Write JSONL results to file
DEMON bench run -m qwen3:8b -e ollama         # Specific model and engine
```
...
```bash
DEMON channel send slack "Hello from DEMON!"
DEMON channel send discord "Build complete"
```
...
```bash
DEMON serve                                 # Default host/port from config
DEMON serve --port 8000                     # Custom port
DEMON serve --host 0.0.0.0 --port 9000      # Bind to all interfaces
DEMON serve --model qwen3:8b                # Specify default model
DEMON serve --agent orchestrator            # Route requests through an agent
```
...
`DEMON` subcommand for it. Construct a `SpecSearchOrchestrator`
directly from `DEMON.learning.spec_search.orchestrator` and call
`.run(trigger)` with a trigger from
`DEMON.learning.spec_search.triggers`. See
