...
class TestDefaults:
    def test_DEMON_config_defaults(self) -> None:
        cfg = DEMONConfig()
...
class TestTomlLoading:
    def test_load_missing_file_uses_defaults(self, tmp_path: Path) -> None:
        ...
        assert isinstance(cfg, DEMONConfig)
    ...
    def test_system_prompt_block_parsed(self, tmp_path: Path) -> None:
        ...
        toml_file = tmp_path / "config.toml"
        toml_file.write_text(
            '[system_prompt]\nprefix = "You are DEMON."\nsoul_max_chars = 999\n'
        )
        cfg = load_config(toml_file)
        assert cfg.system_prompt.prefix == "You are DEMON."
...
class TestSecurityConfig:
    ...
    def test_security_config_on_DEMON_config(self) -> None:
        cfg = DEMONConfig()
...
class TestChannelConfig:
    ...
    def test_channel_config_on_DEMON_config(self) -> None:
        cfg = DEMONConfig()
...
class TestAgentConfigNew:
    ...
    def test_default_system_prompt_anchors_identity(self) -> None:
        """#540: the hardened wording must name DEMON and explicitly
...
class TestSandboxConfig:
    def test_defaults(self) -> None:
        ...
        assert sc.image == "DEMON-sandbox:latest"
    ...
    def test_on_DEMON_config(self) -> None:
        cfg = DEMONConfig()
...
class TestWhatsAppBaileysChannelConfig:
    def test_defaults(self) -> None:
        ...
        assert wc.auth_dir == ""
        assert wc.assistant_name == "DEMON"
