"""Tests for the ``DEMON eval`` CLI commands."""
...
class TestEvalCLI:
    ...
    def test_eval_group_exists(self):
        """``DEMON eval --help`` shows run/compare/report/list subcommands."""
    ...
    def test_eval_list_benchmarks(self):
        """``DEMON eval list`` exits 0 and outputs benchmark names."""
        ...
        assert "DEMON-direct" in result.output
        assert "DEMON-agent" in result.output

    def test_eval_run_missing_args(self):
        """``DEMON eval run`` without required args fails gracefully."""
