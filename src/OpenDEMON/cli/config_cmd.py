...
def _get_config_path(path: str | None) -> Path:
    ...
    if path:
        return Path(path)
    return Path(os.environ.get("DEMON_CONFIG", DEFAULT_CONFIG_PATH))
...
@config.command("path")
def show_path() -> None:
    """Print the resolved DEMON directories (home, config, cache).

    All DEMON state lives under a single root, resolved in priority
    order: ``$DEMON_HOME`` > ``$XDG_DATA_HOME/DEMON`` >
...
def set_config(key: str, value: str) -> None:
    ...
    config_path = Path(
        os.environ.get("DEMON_CONFIG", DEFAULT_CONFIG_DIR / "config.toml")
    )
    if config_path.exists():
