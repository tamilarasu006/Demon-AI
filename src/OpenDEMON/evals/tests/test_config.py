...
class TestExpandSuite:
    ...
    def test_backend_from_benchmark(self):
        suite = EvalSuiteConfig(
            models=[ModelConfig(name="m1")],
            benchmarks=[BenchmarkConfig(name="gaia", backend="DEMON-agent")],
        )
        configs = expand_suite(suite)
        assert configs[0].backend == "DEMON-agent"
...
class TestCLIConfig:
    ...
    def test_run_config_loads_and_prints_suite_info(self, tmp_path):
        ...
        runner = CliRunner()
        with patch("DEMON.evals.cli._run_single", side_effect=Exception("mock")):
