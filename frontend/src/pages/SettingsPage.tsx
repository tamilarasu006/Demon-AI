...
export function SettingsPage() {
  ...
  const [memoryEnabled, setMemoryEnabled] = useState(() => {
    try { return localStorage.getItem('opendemon-memory-enabled') !== 'false'; } catch { return true; }
  });
  const [memoryBackend, setMemoryBackend] = useState(() => {
    try { return localStorage.getItem('opendemon-memory-backend') || 'sqlite'; } catch { return 'sqlite'; }
  });
  const [memoryTopK, setMemoryTopK] = useState(() => {
    try { return parseInt(localStorage.getItem('opendemon-memory-top-k') || '5'); } catch { return 5; }
  });
  const [memoryMinScore, setMemoryMinScore] = useState(() => {
    try { return parseFloat(localStorage.getItem('opendemon-memory-min-score') || '0.1'); } catch { return 0.1; }
  });
  const [memoryMaxTokens, setMemoryMaxTokens] = useState(() => {
    try { return parseInt(localStorage.getItem('opendemon-memory-max-tokens') || '2048'); } catch { return 2048; }
  });
  ...
  const handleExport = () => {
    const data = localStorage.getItem('opendemon-conversations') || '{}';
    const blob = new Blob([data], { type: 'application/json' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = `opendemon-export-${new Date().toISOString().slice(0, 10)}.json`;
    a.click();
    ...
    input.onchange = (e) => {
      ...
      reader.onload = (ev) => {
        try {
          ...
          if (data.version === 1) {
            localStorage.setItem('opendemon-conversations', JSON.stringify(data));
  ...
  const handleClear = () => {
    ...
    localStorage.removeItem('opendemon-conversations');
    useAppStore.getState().loadConversations();
      ...
      <div className="max-w-2xl mx-auto">
        ...
        <div className="flex flex-col gap-4">
          ...
          <Section title="Appearance">
            <SettingRow label="Theme" description="Choose how OpenDemon looks">
          ...
          <Section title="Models">
            ...
            <SettingRow label="Cloud providers" description="Green dot means API key is configured">
              <div className="flex flex-wrap gap-3">
                <CloudProviderStatus label="OpenAI" storageKey="opendemon-openai-key" />
                <CloudProviderStatus label="Anthropic" storageKey="opendemon-anthropic-key" />
                <CloudProviderStatus label="Google" storageKey="opendemon-gemini-key" />
                <CloudProviderStatus label="OpenRouter" storageKey="opendemon-openrouter-key" />
              </div>
            </SettingRow>
          </Section>
          ...
          <Section title="API Keys">
            <SettingRow label="OpenAI" description="GPT-4, GPT-3.5, etc.">
              <ApiKeyInput storageKey="opendemon-openai-key" placeholder="sk-..." />
            </SettingRow>
            <SettingRow label="Anthropic" description="Claude models">
              <ApiKeyInput storageKey="opendemon-anthropic-key" placeholder="sk-ant-..." />
            </SettingRow>
            <SettingRow label="Google" description="Gemini models">
              <ApiKeyInput storageKey="opendemon-gemini-key" placeholder="AI..." />
            </SettingRow>
            <SettingRow label="OpenRouter" description="Multi-provider routing">
              <ApiKeyInput storageKey="opendemon-openrouter-key" placeholder="sk-or-..." />
            </SettingRow>
          </Section>
          ...
          <Section title="Tools">
            <SettingRow label="Web Search" description="SerpAPI or Tavily key for web search tool">
              <ApiKeyInput storageKey="opendemon-search-key" placeholder="API key..." />
            </SettingRow>
          </Section>
            ...
            <SettingRow label="Use memory context" description="Automatically inject relevant memories into conversations">
              <button
                onClick={() => {
                  ...
                  try { localStorage.setItem('opendemon-memory-enabled', String(next)); } catch {}
                  showSaved();
                }}
            ...
            <SettingRow label="Memory backend" description="Which retrieval engine to use">
              <select
                ...
                onChange={(e) => {
                  ...
                  try { localStorage.setItem('opendemon-memory-backend', e.target.value); } catch {}
                  showSaved();
                }}
                className="text-sm px-3 py-1.5 rounded-lg outline-none cursor-pointer"
                style={{
            ...
            <SettingRow label="Results to inject" description={`${memoryTopK}`}>
              <input
                ...
                onChange={(e) => {
                  ...
                  setMemoryTopK(v);
                  try { localStorage.setItem('opendemon-memory-top-k', String(v)); } catch {}
                  showSaved();
                }}
            ...
            <SettingRow label="Min relevance score" description={`${memoryMinScore}`}>
              <input
                ...
                onChange={(e) => {
                  ...
                  setMemoryMinScore(v);
                  try { localStorage.setItem('opendemon-memory-min-score', String(v)); } catch {}
                  showSaved();
                }}
                className="w-32 cursor-pointer accent-[var(--color-accent)]"
              />
            ...
            <SettingRow label="Max context tokens" description={`${memoryMaxTokens}`}>
              <input
                ...
                onChange={(e) => {
                  ...
                  try { localStorage.setItem('opendemon-memory-max-tokens', String(v)); } catch {}
                  showSaved();
                }}
                className="w-32 cursor-pointer accent-[var(--color-accent)]"
              />
          ...
          <Section title="Speech">
            ...
            {!speechBackendAvailable && speechBackendAvailable !== null && (
              <div className="text-xs mt-2 px-1" style={{ color: 'var(--color-text-tertiary)' }}>
                ...
                See the <a href="https://open-jarvis.github.io/OpenDemon/user-guide/tools/" target="_blank" rel="noopener noreferrer" style={{ color: 'var(--color-accent)' }}>documentation</a> for details.
              </div>
            )}
          </Section>
          ...
          <Section title="About">
            <div className="text-sm" style={{ color: 'var(--color-text-secondary)' }}>
              <p className="mb-2">
                <span className="font-semibold" style={{ color: 'var(--color-text)' }}>OpenDemon</span> — Programming abstractions for on-device AI.
              </p>
              ...
              <div className="flex gap-3 mt-3 text-xs">
                <a
                  href="https://scalingintelligence.stanford.edu/blogs/OpenDemon/"
                  target="_blank"
                ...
                <a
                  href="https://open-jarvis.github.io/OpenDemon/"
                  target="_blank"
                  rel="noopener noreferrer"
                  style={{ color: 'var(--color-accent)' }}
                >
