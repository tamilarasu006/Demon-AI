...
This page traces the end-to-end journey of a user query through the DEMON system, from the moment it enters the CLI or SDK to the final response and telemetry recording.
...
sequenceDiagram
    ...
    User->>CLI: DEMON ask "query" / j.ask("query")
    CLI->>CFG: load_config()
    CFG-->>CLI: DEMONConfig (hardware, engine defaults)
...
DEMON supports two query processing paths, selected by the `--agent` CLI flag or the `agent` parameter in the SDK.
...
```bash
# CLI
DEMON ask --agent orchestrator --tools calculator,think "What is 2^10 + 3^5?"
...
```python
config = load_config()  # Reads ~/.DEMON/config.toml
```
...
- Returns a `DEMONConfig` dataclass with all settings
...
The `TelemetryStore` subscribes to `TELEMETRY_RECORD` events on the EventBus and writes records to `~/.DEMON/telemetry.db`.
...
The `DEMON` class in `sdk.py` provides the same query flow through a Python API:

```python
from DEMON import DEMON

j = DEMON(model="qwen3:8b", engine_key="ollama")
