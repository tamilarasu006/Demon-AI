...
class TestDaemonCommands:
    ...
    def test_stop_no_server(self) -> None:
        """``DEMON stop`` when no PID file shows 'not running'."""
        with patch("DEMON.cli.daemon_cmd._read_pid", return_value=None):
            result = CliRunner().invoke(cli, ["stop"])
        assert result.exit_code != 0
        assert "No running server" in result.output

    def test_status_no_server(self) -> None:
        """``DEMON status`` when no PID file shows 'not running'."""
        with patch("DEMON.cli.daemon_cmd._read_pid", return_value=None):
    ...
    def test_read_pid_no_file(self, tmp_path: Path) -> None:
        ...
        with patch(
            "DEMON.cli.daemon_cmd._PID_FILE",
    ...
    def test_write_and_read_pid(self, tmp_path: Path) -> None:
        ...
        with (
            patch("DEMON.cli.daemon_cmd._PID_FILE", pid_file),
            patch("DEMON.cli.daemon_cmd.DEFAULT_CONFIG_DIR", tmp_path),
    ...
    def test_status_shows_running(self) -> None:
        """``DEMON status`` shows running info when PID exists."""
        mock_config = MagicMock()
        mock_config.server.host = "127.0.0.1"
        mock_config.server.port = 8000

        with (
            patch("DEMON.cli.daemon_cmd._read_pid", return_value=9999),
            patch(
                "DEMON.cli.daemon_cmd.load_config",
    ...
    def test_start_already_running(self) -> None:
        """``DEMON start`` exits with error when a server is already running."""
        with patch("DEMON.cli.daemon_cmd._read_pid", return_value=42):
