...
harness — each is registered as a standard DEMON agent so the rest
of the platform (SDK, CLI, distillation, evals) can use them like any
other agent. Results live under ``$DEMON_HYBRID_EXPERIMENTS_DIR``
(defaults to ``~/.DEMON/experiments/hybrid/``).
...
Items in parentheses are what the *paper* trains. These DEMON ports
...
`src/DEMON/evals/scorers/swebench_harness.py` (next to the existing
structural scorer).
...
# 3. Run a smoke cell
.venv/bin/python -m DEMON.agents.hybrid.runner \
    --cell minions-gaia-qwen27b-opus-3
```

Outputs land in
`$DEMON_HYBRID_EXPERIMENTS_DIR/runs/<cell>/{results.jsonl,summary.json,config.json,logs/}`
(defaults to `~/.DEMON-hybrid/experiments/`). The schema matches the
...
```bash
src/DEMON/agents/hybrid/scripts/new_experiment.sh \
...
(`~/.DEMON/experiments/hybrid/docs/results.md`) at full N —
...
these DEMON cells should reproduce the harness numbers within
