...
prove DEMON wiring; this validates that a real H100/H200 host can mine
through Pearl and serve inference through DEMON.
...
- Docker container `DEMON-pearl-miner` starts.
- `~/.DEMON/runtime/mining.json` is written.
...
```bash
docker ps --filter name=DEMON-pearl-miner
cat ~/.DEMON/runtime/mining.json
uv run DEMON mine logs --tail 200
...
## Verify DEMON Inference Uses Mining Endpoint

Run:

```bash
uv run DEMON mine doctor
uv run DEMON ask "Say hello in one sentence."
```
...
- If metric names differ, attach `/tmp/pearl-gateway-metrics.txt` to the PR and
  update `src/DEMON/mining/_metrics.py`.
...
```bash
uv run DEMON mine stop
docker ps --filter name=DEMON-pearl-miner
test ! -e ~/.DEMON/runtime/mining.json
```
...
- Container stops.
- Sidecar is removed.
- `DEMON ask` no longer routes through `vllm-pearl-mining` unless another
...
- DEMON inference succeeds through `vllm-pearl-mining`.
...
```bash
uv run DEMON mine doctor
uv run DEMON mine status || true
uv run DEMON mine logs --tail 300 || true
docker inspect DEMON-pearl-miner || true
