...
The channels module lets DEMON send and receive messages through external messaging platforms. Each platform has a dedicated channel implementation that connects directly to the platform's API -- there is no intermediate gateway.
...
from DEMON.channels.slack import SlackChannel
...
The `DEMON channel` subcommand group provides quick access to channel operations.
...
DEMON channel send telegram "Build completed successfully"
```
...
When `DEMON serve` is running, three channel endpoints are available. Channels must be configured and enabled in `[channel]` for these endpoints to return data.
...
Channel settings live in the `[channel]` section of `~/.DEMON/config.toml`. Each platform has its own nested sub-section.

```toml title="~/.DEMON/config.toml"
[channel]
enabled = true
default_channel = ""
...
# Send a message
if channels:
    channel.send(channels[0], "Hello from DEMON!")
...
```
Your phone  ──text──▶  SendBlue  ──webhook──▶  ngrok tunnel  ──▶  DEMON
                                                                       │
Your phone  ◀──iMessage──  SendBlue  ◀──API call──  DeepResearch agent ◀┘
```

When someone texts the SendBlue number, SendBlue POSTs the message to your webhook. DEMON sends an immediate "Message received!" acknowledgment, runs the DeepResearch agent, and sends the response back via iMessage.
...
Since DEMON runs locally, you need a tunnel so SendBlue can reach your webhook:
...
# Send a message
ok = channel.send("+15551234567", "Hello from DEMON!")
```
...
DEMON channel status --channel-type sendblue

# Send a message
DEMON channel send sendblue "+15551234567" "Hello from DEMON!"
```

### Server Restart Behavior

SendBlue bindings are **automatically restored on server restart**. When `DEMON serve` starts:
...
!!! warning "Node.js 22+ required"
    The Baileys bridge is a compiled Node.js application bundled inside the package. It is auto-installed to `~/.DEMON/whatsapp_baileys_bridge/` on first `connect()` call. If `node` is not found on `PATH`, `connect()` logs an error and sets the channel to `ChannelStatus.ERROR`.
...
On first connection, the bridge will print a QR code to the terminal. Scan it with the WhatsApp app on your phone to authenticate. Authentication state is saved to `~/.DEMON/whatsapp_baileys_bridge/auth/` and reused on subsequent connections.
...
| Parameter                  | Type       | Default     | Description                                            |
|----------------------------|------------|-------------|--------------------------------------------------------|
| `auth_dir`                 | `str`      | `~/.DEMON/whatsapp_baileys_bridge/auth` | Baileys auth state directory |
| `assistant_name`           | `str`      | `"DEMON"`  | Display name for the assistant                         |
...
auth_dir = "/home/user/.DEMON/whatsapp_baileys_bridge/auth"
assistant_name = "DEMON"
...
- [API Reference: Channels](../api-reference/DEMON/channels/index.md) -- full class and type signatures
