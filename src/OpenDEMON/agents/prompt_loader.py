...
- System prompts: ``$DEMON_HOME/agents/{name}/system_prompt.md``
- Few-shot exemplars: ``$DEMON_HOME/agents/{name}/few_shot.json``
...
def _DEMON_home() -> Path:
    """Resolve the DEMON root, honoring DEMON_HOME / XDG_DATA_HOME."""
...
def load_system_prompt_override(agent_name: str) -> str | None:
    ...
    Looks for ``$DEMON_HOME/agents/<agent_name>/system_prompt.md``.
    ``DEMON_HOME`` defaults to ``~/.DEMON`` when unset.
    """
    home = _DEMON_home()
...
def load_few_shot_exemplars(
    ...
    Looks for ``$DEMON_HOME/agents/<agent_name>/few_shot.json``.
