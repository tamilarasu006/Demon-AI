...
class SystemBuilder:
    """Config-driven fluent builder for DEMONSystem."""

    def __init__(
        self,
        config: Optional[DEMONConfig] = None,
        *,
    ...
    def engine_instance(
        ...
        ``DEMON eval --base-url``). ``build()`` health-checks the instance
    ...
    def build(self) -> DEMONSystem:
        """Construct a fully wired DEMONSystem."""
        config = self._config
    ...
    def _resolve_model(self, config: DEMONConfig, engine: InferenceEngine) -> str:
        if self._model:
            return self._model
    ...
    @staticmethod
    def _setup_learning_orchestrator(config: DEMONConfig):
