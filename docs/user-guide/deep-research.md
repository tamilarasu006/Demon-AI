...
This writes a pre-configured `~/.DEMON/config.toml` for the deep research agent.
...
DEMON chunks the content and stores it in a local SQLite/FTS5 database. Supported formats include `.txt`, `.md`, `.pdf`, `.py`, `.json`, `.csv`, and more.
...
DEMON ask "What meetings did I have with Alice last month?"

# Explicitly specify the agent
DEMON ask --agent deep_research "Compare the approaches described in paper-a.pdf and paper-b.pdf"

# Index more documents
DEMON memory index ~/Downloads/reports/
DEMON memory index ./notes.md

# Search memory directly
DEMON memory search "project timeline"
DEMON memory search -k 20 "budget estimates"
...
DEMON ask "Summarize all emails about the Q3 budget review"

# Cross-reference sources
DEMON ask "What do papers A and B agree on regarding attention mechanisms?"

# Find specific information
DEMON ask "What meetings did I have with Alice last month?"

# Extract structured data
DEMON ask "List all action items from the meeting notes in ~/Documents/meetings/"

# Research with web fallback
DEMON ask "Compare our internal benchmarks with the latest published results"
```
...
# Single file
DEMON memory index ./report.pdf

# Custom chunk size for long documents
DEMON memory index ./paper.pdf --chunk-size 1024 --chunk-overlap 128
```
...
```bash
curl -s https://example.com/article | DEMON memory index --stdin --source "example.com"
```

### Code repositories

```bash
DEMON memory index ./src/ --chunk-size 256
```
...
**"No results found"** -- Make sure you have indexed documents first with `DEMON memory index`. Check indexed content with `DEMON memory stats`.
