...
_AVAIL = "DEMON.mining._install.pearl_packages_available"
...
def test_detect_supported_on_apple_silicon_when_packages_present(darwin_apple_hw):
    from OpenDEMON.mining.cpu_pearl import CpuPearlProvider
...
def test_start_writes_sidecar_and_is_running(cpu_pearl_config, tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("DEMON.mining.cpu_pearl._sidecar_path", lambda: sidecar)
    monkeypatch.setattr("DEMON.mining.cpu_pearl._log_dir", lambda: log_dir)
    ...
    _target = "DEMON.mining.cpu_pearl.PearlSubprocessLauncher"
...
def test_parse_gateway_metrics_extracts_counters():
    ...
    from OpenDEMON.mining.cpu_pearl import _parse_gateway_metrics
