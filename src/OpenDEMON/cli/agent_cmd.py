...
def list_agents() -> None:
    ...
    try:
        ...
        if not agents:
            console.print(
                "[dim]No agents found. Create one with: DEMON agents create[/dim]"
            )
            return
...
def _get_scheduler_and_executor(system=None):
    """Get scheduler + executor from a DEMONSystem instance."""
    if system is None:
        system = _get_system()
...
def run_agent(agent_id):
    ...
    if not instruction and not pending:
        click.echo(
            ...
            f"'DEMON agents ask {agent_id} <message>' to set one.",
            err=True,
        )
        raise SystemExit(1)
