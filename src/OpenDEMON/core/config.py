...
if TYPE_CHECKING:
    # Only used by type-checkers (mypy/pyright) for the ``DEMONConfig.mining``
...
# once at import via the env-aware resolver in ``DEMON.core.paths`` (the
# install-script model: ``DEMON_HOME`` / ``XDG_DATA_HOME`` are set before
...
class ACEOptimizerConfig:
    ...
    Install via ``pip install -e DEMON[learning-ace]`` once the
    ...
    # most DEMON users have credentials for.
    ...
    # Empty string defaults to ``~/.DEMON/learning/ace/<task>/``.
    save_dir: str = ""
    task_name: str = "DEMON"
...
class AnalyticsConfig:
    ...
    usage events sent to the DEMON team's PostHog instance to
...
class WhatsAppBaileysChannelConfig:
    ...
    auth_dir: str = ""  # Defaults to ~/.DEMON/whatsapp_auth
    assistant_name: str = "DEMON"
...
class SandboxConfig:
    ...
    enabled: bool = False
    image: str = "DEMON-sandbox:latest"
    timeout: int = 300
    workspace: str = ""
...
class SchedulerConfig:
    ...
    db_path: str = ""  # Defaults to ~/.DEMON/scheduler.db
...
class SkillsConfig:
    ...
    index_repo: str = "https://github.com/DEMON/skill-index.git"
    index_dir: str = field(
...
# Sections that users may set via ``DEMON config set``.
# ``hardware`` is auto-detected and not user-settable.
_SETTABLE_SECTIONS = frozenset(DEMONConfig.__dataclass_fields__.keys()) - {
    "hardware",
    "mining",
}
...
def validate_config_key(dotted_key: str) -> type:
    ...
    The function walks the ``DEMONConfig`` dataclass hierarchy using
    ``dataclasses.fields()``.
...
def _migrate_toml_data(data: Dict[str, Any], cfg: "DEMONConfig") -> None:
...
def load_config(path: Optional[Path] = None) -> DEMONConfig:
    """Detect hardware, build defaults, overlay TOML overrides.

    Parameters
    ----------
    path:
        Explicit config file. If not set, uses ``DEMON_CONFIG`` when set,
        otherwise ``~/.DEMON/config.toml``.
    """
    _ensure_config_dir()
    hw = detect_hardware()
    cfg = DEMONConfig(hardware=hw)
    cfg.engine.default = recommend_engine(hw)

    if path is not None:
        config_path = Path(path)
    elif os.environ.get("DEMON_CONFIG"):
        config_path = Path(os.environ["DEMON_CONFIG"]).expanduser().resolve()
    else:
...
# ---------------------------------------------------------------------------
# Default TOML generation (for ``DEMON init``)
# ---------------------------------------------------------------------------
...
# Full reference config: DEMON init --full
...
def generate_default_toml(
    ...
    """Render a commented TOML string suitable for ``~/.DEMON/config.toml``."""
    engine = engine or recommend_engine(hw)
    model = recommend_model(hw, engine)
    gpu_line = ""
    if hw.gpu:
...
# enabled = false
# poll_interval = 60
# db_path = ""                # Defaults to ~/.DEMON/scheduler.db

# [channel.whatsapp_baileys]
# auth_dir = ""               # Defaults to ~/.DEMON/whatsapp_auth
