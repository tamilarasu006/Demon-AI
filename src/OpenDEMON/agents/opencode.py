...
handles the agentic loop / tools, DEMON supplies the model.
...
def _derive_openai_base_url(engine: Any) -> str:
    """Best-effort OpenAI-compatible base URL for an DEMON engine.
...
def _extract_tool_results(parts: List[dict]) -> List[ToolResult]:
    """Map opencode ``tool`` parts to DEMON ToolResults (best-effort)."""
...
class OpenCodeAgent(BaseAgent):
    ...
    inference runs on DEMON's selected local model. ``agent`` selects
    ...
    def _ensure_server(self) -> str:
        ...
        self._config_dir = tempfile.mkdtemp(prefix="DEMON-opencode-")
