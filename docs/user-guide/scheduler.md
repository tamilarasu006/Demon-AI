...
!!! note "Optional component"
    The scheduler is a standalone module (`DEMON.scheduler`). It is not wired into the default `DEMON` / `DEMONSystem` startup. You enable it explicitly via `SystemBuilder` or by starting the CLI daemon with `DEMON scheduler start`.
...
The `DEMON scheduler` subcommand group manages tasks and the daemon from the terminal.
...
# Active tasks only
DEMON scheduler list --status active

# Paused tasks
DEMON scheduler list --status paused
```
...
DEMON scheduler pause a3f9b12c4d8e

# Resume -- next_run is recomputed from the current time
DEMON scheduler resume a3f9b12c4d8e
```
...
DEMON scheduler cancel a3f9b12c4d8e
```
...
DEMON scheduler logs a3f9b12c4d8e
```
...
# Set up storage
store = SchedulerStore(db_path="~/.DEMON/scheduler.db")  # (1)!

# Wire in a DEMONSystem for task execution
...
4. Starts a daemon thread named `"DEMON-scheduler"`. Daemon threads exit automatically when the main process exits.
...
Scheduler settings live in the `[scheduler]` section of `~/.DEMON/config.toml`.

```toml title="~/.DEMON/config.toml"
[scheduler]
enabled = false
db_path = "~/.DEMON/scheduler.db"
poll_interval = 60
...
| `db_path`        | `str`  | `~/.DEMON/scheduler.db`     | SQLite database path                       |
