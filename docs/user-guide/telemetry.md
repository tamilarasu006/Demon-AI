...
DEMON has two complementary observability systems: **telemetry** for per-inference metrics and **traces** for full interaction-level recording. Together, they provide comprehensive insight into system behavior and power the learning system's routing policy updates.
...
bus = EventBus()
store = TelemetryStore(db_path="~/.DEMON/telemetry.db")
...
agg = TelemetryAggregator(db_path="~/.DEMON/telemetry.db")
...
DEMON telemetry stats -n 5          # Top 5 models only

# Export records
DEMON telemetry export              # JSON to stdout
DEMON telemetry export -f csv       # CSV to stdout
DEMON telemetry export -o data.json # JSON to file
DEMON telemetry export -f csv -o metrics.csv

# Clear all records
DEMON telemetry clear               # With confirmation prompt
DEMON telemetry clear --yes         # Without confirmation
```
