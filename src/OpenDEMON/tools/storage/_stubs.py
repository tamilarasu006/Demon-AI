...
#: constructed because the mandatory ``DEMON_rust`` extension is missing
...
RUST_MISSING_HINT = (
    "Memory backend unavailable: the native `DEMON_rust` extension is not "
...
class MemoryBackendUnavailable(RuntimeError):
    ...
    ``DEMON_rust`` extension is missing from the current environment.
