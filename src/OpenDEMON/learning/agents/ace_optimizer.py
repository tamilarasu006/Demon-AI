...
The wrapper adapts DEMON traces into ACE's
...
sidecar overlay under ``~/.DEMON/learning/ace/<task>/`` for the
...
class _TraceDataProcessor:
    """Adapter that exposes DEMON traces in ACE's three-method API.
...
def _traces_to_samples(traces: List[Any]) -> List[Dict[str, Any]]:
    """Convert DEMON trace records to ACE's sample dict shape."""
    ...
    def optimize(self, trace_store: Any) -> Dict[str, Any]:
        ...
        if not HAS_ACE:
            return {
                "status": "error",
                "reason": (
                    "ace not installed (pip install 'DEMON[learning-ace]')"
                ),
            }
