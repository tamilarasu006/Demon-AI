...
This writes a pre-configured `~/.DEMON/config.toml` for the operative agent with scheduling support.
...
DEMON scheduler status <task-id>

# Pause / resume / delete a task
DEMON scheduler pause <task-id>
DEMON scheduler resume <task-id>
DEMON scheduler delete <task-id>

# Run a task immediately (outside its schedule)
DEMON scheduler run <task-id>

# Stop the scheduler daemon
DEMON scheduler stop
```
...
- **All data stays local**: State is stored in `~/.DEMON/` using the configured memory backend. Nothing leaves your machine.

## Troubleshooting

**"Scheduler not running"** -- Start the scheduler daemon with `DEMON scheduler start`. It must be running for scheduled tasks to execute.

**Task doesn't run on time** -- Check that Ollama is running (`ollama serve`). The scheduler triggers the agent, but the agent needs an inference engine. Verify the schedule with `DEMON scheduler status <task-id>`.
...
**Memory grows too large** -- Periodically review with `DEMON memory stats`. Clear old entries with `DEMON memory clear --before 2026-01-01` if needed.
