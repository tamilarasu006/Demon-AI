...
impl PyAgentCard {
    ...
    fn new(name: &str, description: &str, version: &str, url: &str) -> Self {
        Self {
            inner: DEMON_a2a::AgentCard::new(name, description, version, url),
        }
    }
...
impl PyA2ATaskStore {
    ...
    fn new() -> Self {
        Self {
            inner: DEMON_a2a::A2ATaskStore::new(),
        }
    }
    ...
    fn update_state(&mut self, id: &str, state: &str) -> bool {
        let s = match state {
            "pending" => DEMON_a2a::TaskState::Pending,
            "active" => DEMON_a2a::TaskState::Active,
            "completed" => DEMON_a2a::TaskState::Completed,
            "cancelled" => DEMON_a2a::TaskState::Cancelled,
            "failed" => DEMON_a2a::TaskState::Failed,
            _ => return false,
        };
...
pub fn parse_a2a_request(json_str: &str) -> PyResult<String> {
    let req = DEMON_a2a::parse_request(json_str)
