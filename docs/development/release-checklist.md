...
2. **`DEMON` (no args)** drops into a chat session within 2 s.
...
6. **`DEMON doctor`** exits 0 once all bg work completes; shows the Background tasks table.
7. **Re-run `curl … | bash`** on the same machine. It completes ≤ 30 s, says `[ok] step already done` for every step.
8. **`DEMON-uninstall`** removes `~/.DEMON/` and `~/.local/bin/DEMON*`. Verify with `ls`.
...
- Delete `~/.DEMON/config.toml` — verify bare `DEMON` re-runs init.
- Delete `~/.DEMON/.venv` — verify re-running curl heals it.
