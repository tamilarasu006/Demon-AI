...
def doctor() -> None:
    ...
    if mining_cfg is None:
        _row("RPC", False, "no [mining] config - run `DEMON mine init`")
    else:
...
def init(
        ...
        if local_model_path is None:
            ...
            if not cap.supported:
                raise click.ClickException(
                    ...
                    "See `DEMON mine models` and `DEMON mine doctor` for details."
                )
    ...
    if pearld_password_env not in os.environ:
        click.echo(
            ...
            "Set it before `DEMON mine start`.",
            err=True,
        )
...
"""
    ...
    click.echo("Done. Run `DEMON mine start` to begin mining.")
...
def start() -> None:
    ...
    if cfg is None:
        raise click.ClickException(
            "no [mining] section in config - run `DEMON mine init`"
        )
    provider = MinerRegistry.get(cfg.provider)()
    asyncio.run(provider.start(cfg))
    click.echo(f"Started {cfg.provider}. Run `DEMON mine status` for live stats.")
...
def validate_model(
    ...
    if sidecar is None:
        record(
            "Sidecar",
            False,
            "absent - run `DEMON mine start` first",
        )
    else:
...
def logs(tail_n: int, follow: bool) -> None:
    ...
    try:
        launcher._container = client.containers.get("DEMON-pearl-miner")
    except Exception as exc:  # noqa: BLE001
