"""``DEMON host`` — download and serve a model locally with auto backend setup."""
...
def host(
    ...
    \b
      DEMON host mlx-community/Qwen2.5-7B-4bit --backend mlx
      DEMON host Qwen/Qwen3-8B --backend vllm
      DEMON host qwen3:8b --backend ollama
      DEMON host meta-llama/Llama-3-8B -b sglang
    """
    console = Console()
    ...
    if backend != "ollama":
        ...
        console.print(
            "[dim]DEMON will auto-discover it. Press Ctrl+C to stop.[/dim]\n"
        )
