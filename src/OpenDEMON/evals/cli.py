...
def _build_backend(
    ...
    - "DEMON-direct" and "DEMON-agent" honor them when
...
def _run_terminalbench_native(
    ...
    *,
    ...
    ``base_url`` (from ``--base-url`` / DEMON_BACKEND_BASE_URL) targets an
...
def _run_single(
    ...
    *,
    ...
    base_url = (
        ...
        or os.environ.get("DEMON_BACKEND_BASE_URL")
    )
    api_key = (
        getattr(config, "api_key", None)
        or _metadata.get("api_key")
        or os.environ.get("DEMON_BACKEND_API_KEY")
    )
...
@click.option(
    ...
    help=(
        ...
        "hermes/openclaw; for DEMON-direct/DEMON-agent/terminalbench-native "
        "it bypasses engine discovery and targets this URL directly "
        "(env: DEMON_BACKEND_BASE_URL)."
    ),
)
@click.option(
    "--api-key",
...
@click.option(
    ...
    help="Agent name for DEMON-agent backend",
)
...
def run(
    ...
    config = RunConfig(
        ...
        base_url=base_url or os.environ.get("DEMON_BACKEND_BASE_URL"),
        api_key=api_key or os.environ.get("DEMON_BACKEND_API_KEY"),
        metadata={
            "base_url": base_url or os.environ.get("DEMON_BACKEND_BASE_URL"),
            "api_key": api_key or os.environ.get("DEMON_BACKEND_API_KEY"),
        },
...
def run_all(
    ...
    for i, bench_name in enumerate(BENCHMARKS, 1):
        ...
        eval_backend = _build_backend("DEMON-direct", engine_key, "orchestrator", [])
