...
def write_initial_config(
    *,
    ...
    Called by both ``install.sh`` (via ``DEMON _bootstrap --write-config``)
    and ``DEMON init`` so the TOML format has one definition.
    """
    ...
    base_toml = (
        f"# DEMON configuration\n"
        f"{hardware_line}{gpu_comment}\n"
        f"# Full reference config: DEMON init --full\n"
        f"\n"
        f"{provenance}\n"
        f"\n"
        f"[engine]\n"
...
# ---------------------------------------------------------------------------
# CLI command — invoked by install.sh, hidden from `DEMON --help`
# ---------------------------------------------------------------------------
