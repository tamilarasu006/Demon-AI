...
2. Tolerant pass — maps non-spec top-level fields to their canonical
   locations under metadata.DEMON.* via FIELD_MAPPING.
...
# dataclass attribute.  When target_kind is "DEMON_meta" the value is
# stored under manifest.metadata["DEMON"][attr].
FIELD_MAPPING: Dict[str, tuple[str, str]] = {
    ...
    "platforms": ("DEMON_meta", "platforms"),
    "prerequisites": ("DEMON_meta", "prerequisites"),
}
...
class SkillParser:
    ...
    def _build_manifest(
        ...
        captures unknown fields under metadata.DEMON.original_frontmatter,
        and merges metadata.DEMON.* into canonical fields.
        """
        ...
        if not isinstance(raw_metadata, dict):
            raw_metadata = {}
        # Initialize DEMON namespace
        oj_meta = dict(raw_metadata.get("DEMON") or {})
        ...
        for key, value in frontmatter.items():
            ...
            if key in FIELD_MAPPING:
                ...
                LOGGER.warning(
                    ...
                    "(value preserved in metadata.DEMON.original_frontmatter)",
                    key,
                    manifest.name,
                )

        # Merge metadata.DEMON.* into canonical fields (these override
        # top-level mappings since they are explicit DEMON-namespaced).
        for key in (
            "version",
            "author",
            "tags",
        ...
        # Preserve unmapped fields under metadata.DEMON.original_frontmatter
        if unmapped:
            oj_meta["original_frontmatter"] = unmapped

        # Stash the DEMON metadata block back
        if oj_meta:
            new_metadata = dict(raw_metadata)
            new_metadata["DEMON"] = oj_meta
