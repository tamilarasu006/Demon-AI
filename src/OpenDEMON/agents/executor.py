...
class AgentExecutor:
    ...
    Constructor receives a DEMONSystem reference for access to engine,
    ...
    def set_system(self, system: Any) -> None:
        """Deferred system injection — called after DEMONSystem is constructed."""
        self._system = system
    ...
    def _invoke_agent(self, agent: dict) -> AgentResult:
        ...
        # Resolve engine + model from DEMONSystem
        engine = self._system.engine if self._system else None
        if engine is None:
            raise FatalError("No engine available in DEMONSystem")
        model = (
            config.get("model")
        ...
        # agent's own ToolExecutor. Set by CLI paths like `DEMON agents ask`
        ...
        if self._system is not None:
            ...
            # agents, mirroring the one-shot `DEMON ask` path so they no
