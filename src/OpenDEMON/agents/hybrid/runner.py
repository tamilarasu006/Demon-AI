...
the registered agent, loads bench tasks via DEMON's existing dataset
...
existing rescore / dashboard scripts can read DEMON cells without
modification.
"""
...
# ``DEMON_HYBRID_TASK_TIMEOUT_S`` (0 / negative disables).
DEFAULT_TASK_TIMEOUT_S = float(
    os.environ.get("DEMON_HYBRID_TASK_TIMEOUT_S", "1800") or 1800
)
...
def load_registry(registry_dir: Optional[Path] = None) -> Dict[str, Dict[str, Any]]:
    ...
    env_override = os.environ.get("DEMON_HYBRID_REGISTRY_DIR")
    if env_override:
...
def _load_gaia_tasks(n: Optional[int]) -> List[Dict[str, Any]]:
    ...
    for rec in ds.iter_records():
        ...
        # DEMON prefixes with `gaia-`) so subsets keyed by the upstream
        # id round-trip.
        md = rec.metadata or {}
...
def _get_gaia_scorer():
    ...
    ``DEMON_GAIA_JUDGE_MODEL``); the judge backend is the ``cloud``
    engine, so ``DEMON_CONFIG`` needs a ``[engine.cloud]`` section.
    """
    global _GAIA_SCORER
    if _GAIA_SCORER is None:
        with _GAIA_SCORER_LOCK:
            if _GAIA_SCORER is None:
                ...
                try:
                    backend = DEMONDirectBackend(engine_key="cloud")
...
def _score_gaia(task: Dict[str, Any], answer: str) -> Dict[str, Any]:
    ...
    Uses the shared DEMON :class:`GAIAScorer`. The previous version
...
def _run_cell_locked(
    ...
    *,
    ...
    # (env ``DEMON_HYBRID_TASK_TIMEOUT_S``, 1800s) applies. 0 disables.
    mcfg = cell.get("method_cfg") or {}
