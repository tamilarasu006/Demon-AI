#!/usr/bin/env python3
"""DEMON Twitter Bot — @DEMONAI reactive mention handler.
...
DEMO_TWEETS = [
    {
        ...
        "author": "alice_dev",
        "text": "@DEMONAI how do I add a new channel integration?",
    },
    {
        "id": "1000000000000000002",
        "author": "bob_user",
        "text": (
            "@DEMONAI bug: the memory_search tool crashes "
    ...
    },
    {
        ...
        "author": "carol_eng",
        "text": "@DEMONAI it would be great to have a built-in scheduler UI",
    },
    {
        "id": "1000000000000000004",
        "author": "dave_fan",
        "text": "@DEMONAI just discovered this project, absolutely love it!",
    },
    {
        "id": "1000000000000000005",
        "author": "spambot99",
        "text": "@DEMONAI BUY CRYPTO NOW 🚀🚀🚀 LINK IN BIO",
    },
]
...
# nomic-embed-text on the DEMON fixture corpus, relevant queries
...
def _build_question_grounded_prompt(
    ...
    return (
        "You are @DEMONAI. Someone asked a question. We retrieved "
...
def _build_question_deferral_prompt(author: str, tweet_id: str, text: str) -> str:
    ...
    """
    return (
        "You are @DEMONAI. Someone asked a question, but our docs "
...
def _build_bug_prompt(author: str, tweet_id: str, text: str) -> str:
    return (
        "You are @DEMONAI. Someone reported a bug.\n\n"
        f"Tweet from @{author} (tweet ID: {tweet_id}):\n"
        f'"{text}"\n\n'
        "1. call http_request to create a github issue:\n"
        "   url: https://api.github.com/repos/DEMON/DEMON/issues\n"
        "   method: POST\n"
...
def _build_feature_prompt(author: str, tweet_id: str, text: str) -> str:
    return (
        "You are @DEMONAI. Someone requested a feature.\n\n"
        f"Tweet from @{author} (tweet ID: {tweet_id}):\n"
        f'"{text}"\n\n'
        "1. call http_request to create a github issue:\n"
        "   url: https://api.github.com/repos/DEMON/DEMON/issues\n"
...
def _build_praise_prompt(author: str, tweet_id: str, text: str) -> str:
    return (
        "You are @DEMONAI. Someone said something nice.\n\n"
        f"Tweet from @{author} (tweet ID: {tweet_id}):\n"
        f'"{text}"\n\n'
...
_INJECTION_PROMPT = (
    "Classify this tweet mentioning @DEMONAI as SAFE or MALICIOUS. "
...
def _classify_mention(text: str, DEMON) -> str:
    ...
    output.
    """
    llm_label = _classify_mention_llm(text, DEMON)
...
def _run_demo(model: str, engine_key: str) -> None:
    ...
    try:
        ...
        click.echo(
            "Error: DEMON is not installed. "
            "Install it with:  uv sync --extra dev",
            err=True,
        )
        sys.exit(1)

    click.echo("DEMON Twitter Bot — Demo Mode (reactive only)")
    click.echo(f"Model: {model}  |  Engine: {engine_key}")
    click.echo("=" * 60)

    try:
        j = DEMON(model=model, engine_key=engine_key)
    except Exception as exc:
        click.echo(
            f"Error: could not initialize DEMON — {exc}\n\n"
    ...
    try:
        for idx, tweet in enumerate(DEMO_TWEETS, 1):
            mention_type = _classify_mention(tweet["text"], DEMON=j)
            click.echo(
...
_SINCE_ID_STATE_PATH = Path.home() / ".DEMON" / "twitter_since_id.txt"
...
def _seed_since_id_to_newest(channel) -> Optional[str]:
    ...
    1. **Persisted state from a prior run** (``~/.DEMON/twitter_since_id.txt``).
...
def _run_live(
    ...
    *,
    ...
    click.echo(f"DEMON Twitter Bot — {mode_label} Mode")
    click.echo(f"Model: {model}  |  Engine: {engine_key}")
    click.echo("=" * 60)
    ...
    try:
        ...
        click.echo(f"Error: could not initialize DEMON — {exc}", err=True)
        sys.exit(1)
    ...
    if dry_run:
        ...
        def _dry_http_execute(self, **params):  # noqa: ANN001
            ...
            return ToolResult(
                ...
                content=(
                    '{"number": 999, "html_url": '
                    '"https://github.com/DEMON/DEMON/issues/999"}'
                ),
            )
    ...
    click.echo(f"\n{mode_hint}")
    click.echo("Waiting for @DEMONAI mentions (poll every 60s). Ctrl+C to stop.\n")
    ...
    def _handle_mention(msg):  # noqa: ANN001
        ...
        if _detect_injection(msg.content, DEMON=j) == "MALICIOUS":
            click.echo(
        ...
        mention_type = _classify_mention(msg.content, DEMON=j)
...
def main(
    ...
    """DEMON Twitter bot — reactive @DEMONAI mention handler.
