...
DEMON has two types of integrations:
...
```json
{
    "display_information": { "name": "DEMON" },
    "features": {
        "app_home": {
        ...
        "bot_user": { "display_name": "DEMON", "always_online": true }
    },
    "oauth_config": {
        "scopes": {
            "bot": [
...
- You DM @DEMON in Slack → Socket Mode receives the event in real-time
...
| Bot not visible in Slack | Click "+" next to Direct Messages → search "DEMON" |
...
3. **Connect in DEMON:**
   ...
   - CLI: `uv run DEMON connect gmail_imap`
...
Slack serves two purposes in DEMON:
...
8. **(Optional) Set the app icon:**
   ...
   - Upload the [DEMON icon](https://github.com/DEMON/DEMON/blob/main/assets/DEMON-slack-icon.jpg)
...
- CLI: `uv run DEMON connect slack`
...
**DM your agent:**
- In Slack, find **DEMON** under Apps (or Direct Messages)
- If you don't see it: click "+" next to Direct Messages → search "DEMON"
...
- **Channel visibility:** The bot can only read channels it's been added to. Invite it with `/invite @DEMON` in each channel you want indexed.
...
| No messages found (data source) | The bot can only see channels it's been added to. Invite it: `/invite @DEMON` in the channel |
...
3. **Connect in DEMON:**
   ...
   - CLI: `uv run DEMON connect notion`
...
3. **Connect in DEMON:**
   ...
   - CLI: `uv run DEMON connect granola`
...
1. **Grant Full Disk Access** to your terminal app:
   ...
   - Enable access for Terminal, iTerm, Warp, or the DEMON desktop app
...
3. **Connect in DEMON:**
   ...
   - CLI: `uv run DEMON connect outlook`
...
2. **Connect in DEMON:**
   ...
   - CLI: `uv run DEMON connect obsidian --path /path/to/vault`
...
4. **Connect in DEMON:**
   ...
   - CLI: `uv run DEMON connect dropbox`
...
| Issue | Solution |
|-------|----------|
| "Connected — no data synced yet" | The connector authenticated but hasn't synced. Try running `uv run DEMON deep-research-setup --skip-chat` to trigger a sync. |
| Data seems stale | Connectors sync on demand. Run the setup command or click "Reconnect" to re-sync. |
| Want to reset a connector | Click "Reconnect" in the Channels tab, or delete the credential file at `~/.DEMON/connectors/{connector}.json` |

### Where credentials are stored

All credentials are saved locally at `~/.DEMON/connectors/` with file permissions `0600` (owner-only read/write). No credentials are sent to any server — everything stays on your device.
