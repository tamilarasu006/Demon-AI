...
def test_doctor_exit_code_when_bg_failed(tmp_DEMON_home: Path) -> None:
    (tmp_DEMON_home / ".state" / "extension-failed").write_text("oom")
    runner = CliRunner()
    result = runner.invoke(doctor, [], catch_exceptions=False)
...
def test_doctor_no_bg_section_when_state_dir_empty(tmp_DEMON_home: Path) -> None:
