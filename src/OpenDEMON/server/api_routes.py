...
async def list_agents(request: Request):
    ...
    registered = []
    try:
        import DEMON.agents  # noqa: F401 — side-effect registration
    ...
    running = []
    try:
        from DEMON.tools.agent_tools import _SPAWNED_AGENTS
...
async def create_agent(req: AgentCreateRequest, request: Request):
    """Spawn a new agent."""
    try:
        from DEMON.tools.agent_tools import AgentSpawnTool
...
async def kill_agent(agent_id: str, request: Request):
    ...
    try:
        from DEMON.tools.agent_tools import AgentKillTool
...
async def message_agent(agent_id: str, req: AgentMessageRequest, request: Request):
    ...
    try:
        from DEMON.tools.agent_tools import AgentSendTool
...
def _get_memory_backend(request: Request):
    ...
    cannot be built because the mandatory ``DEMON_rust`` extension is not
    ...
    if backend is None:
        from DEMON.tools.storage._stubs import MemoryBackendUnavailable
...
async def memory_config(request: Request):
    ...
    ``backend_type: sqlite``) when the native ``DEMON_rust`` extension is
...
async def memory_index(req: MemoryIndexRequest, request: Request):
    ...
    try:
        ...
        # Sandbox: when workspace roots are configured via DEMON_WORKSPACE
        ...
        workspace = os.environ.get("DEMON_WORKSPACE", "").strip()
        if workspace:
            roots = [
...
async def list_skills(request: Request):
    ...
    try:
        from DEMON.core.registry import SkillRegistry
...
async def install_skill(request: Request):
    ...
    return {
        ...
        "message": "Use TOML files in ~/.DEMON/skills/",
    }
...
async def list_sessions(request: Request, limit: int = 20):
    ...
    try:
        from DEMON.sessions.store import SessionStore
...
async def prometheus_metrics(request: Request):
    ...
    try:
        ...
        lines = [
            "# HELP DEMON_requests_total Total requests processed",
            "# TYPE DEMON_requests_total counter",
            f"DEMON_requests_total {stats.get('total_requests', 0)}",
            "# HELP DEMON_tokens_total Total tokens generated",
            "# TYPE DEMON_tokens_total counter",
            f"DEMON_tokens_total {stats.get('total_tokens', 0)}",
            "# HELP DEMON_latency_avg_ms Average latency in milliseconds",
            "# TYPE DEMON_latency_avg_ms gauge",
            f"DEMON_latency_avg_ms {stats.get('avg_latency_ms', 0)}",
        ]
        from starlette.responses import PlainTextResponse
...
def include_all_routes(app) -> None:
    ...
    from DEMON.server.approval_routes import (
    ...
    try:
        if hasattr(app.state, "agent_manager") and app.state.agent_manager:
            from DEMON.server.agent_manager_routes import (  # noqa: PLC0415
