...
- An OpenAI-compatible mock server reachable at $DEMON_MOCK_LLM_URL
...
class TestLiveExternal:
    def test_hermes_runner_emits_valid_json(self, tmp_path: Path) -> None:
        ...
        result = subprocess.run(
            [
                ...
                "test",
                "--base-url",
                os.environ.get("DEMON_MOCK_LLM_URL", "http://localhost:8000/v1"),
                "--api-key",
                "dummy",
                "--api-mode",
                "chat_completions",
    ...
    def test_openclaw_runner_emits_valid_json(self, tmp_path: Path) -> None:
        runner = (
            Path(__file__).resolve().parents[3]
            / "src/DEMON/evals/backends/external/_runners/openclaw_runner.mjs"
        )
