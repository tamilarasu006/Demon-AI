"""``DEMON eval`` — evaluation framework CLI commands."""
...
@click.option(
    ...
    type=click.Choice(
        ["DEMON-direct", "DEMON-agent", "hermes", "openclaw", "terminalbench-native"]
    ),
    help=(
        "Inference backend. For hermes/openclaw, also pass --base-url and "
        "--api-key (or set DEMON_BACKEND_BASE_URL/DEMON_BACKEND_API_KEY)."
    ),
)
@click.option(
    "--base-url",
...
def eval_run(
    ...
    config = RunConfig(
        ...
        # DEMON_BACKEND_BASE_URL/DEMON_BACKEND_API_KEY.
