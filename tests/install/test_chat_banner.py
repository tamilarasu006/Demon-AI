...
def test_banner_shows_rust_building(tmp_DEMON_home: Path) -> None:
...
def test_banner_shows_model_downloading(tmp_DEMON_home: Path) -> None:
    (tmp_DEMON_home / ".state" / "extension-built").write_text("")
...
def test_banner_shows_failed_in_dim_warning(tmp_DEMON_home: Path) -> None:
    (tmp_DEMON_home / ".state" / "extension-failed").write_text("error tail")
    s = _bg_state.get_status()
    banner = render_startup_banner(s)
