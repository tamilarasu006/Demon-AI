...
def hint_no_config() -> str:
    ...
    return (
        ...
        "  Run [bold]DEMON init[/bold] to detect hardware and generate "
        "[cyan]~/.DEMON/config.toml[/cyan].\n"
        "  Or run [bold]DEMON quickstart[/bold] for a guided setup."
    )
...
def hint_no_engine(engine_name: Optional[str] = None) -> str:
    ...
    return (
        ...
        "  Run [bold]DEMON doctor[/bold] to check all engines.\n"
        "  Run [bold]DEMON quickstart[/bold] for guided setup.\n"
        "\n"
        "  [dim]To use a remote engine:[/dim]\n"
        f"    [cyan]DEMON config set engine.{name}.host http://<remote-ip>:<port>[/cyan]\n"
...
def hint_no_model(model_name: Optional[str] = None) -> str:
    ...
    if model_name:
        return (
            ...
            "  Run [bold]DEMON model list[/bold] to see available models."
        )
    return (
