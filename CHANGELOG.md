...
All notable changes to DEMON are documented in this file.
...
**Vision input for `DEMON ask`** — attach images to a query with
...
`DEMON_NUM_CTX` environment variable to tune the Ollama context window
(default `16384`).
...
`src/DEMON/traces/` — silently dropping the whole package. Every
fresh `pip install DEMON==1.0.1` then failed at import with
`ModuleNotFoundError: No module named 'DEMON.traces'` on the
first `DEMON ask`, learning, or server call. Anchored the pattern to
...
the v1.0.1 cutoff, so v1.0.1 users still saw `0.0 GB` from `DEMON
...
`DEMON.ai/install.sh` URL was failing with `sslv3 alert
...
`https://DEMON.github.io/DEMON/install.sh`, generated from
...
**`DEMON self-update`** subcommand. Detects how DEMON was
installed (pip, uv tool, editable git checkout) by inspecting
`DEMON.__file__`, then runs the right upgrade command. Supports
...
(`DEMON_NO_UPDATER=1`) documented in `docs/desktop-auto-update.md`.

**Analytics env-var opt-out** (`DO_NOT_TRACK`, `DEMON_NO_ANALYTICS`).
...
upgrade command. Honors `DEMON_NO_UPDATE_CHECK=1` and `CI=true` to
...
- **Editable-git users running `DEMON self-update`** get the
  detected `git pull && uv sync` command pointed at their actual
  checkout, not `~/DEMON`. If you'd come to rely on the
...
Companion blog post:
[From Minions to DEMON: A Retrospective on Two Years in Local AI](https://hazyresearch.stanford.edu/blog/2026-05-19-minions-to-DEMON-retrospective).
...
`DEMONSystem` in `src/DEMON/system.py`, driven by a TOML config.
...
`DEMON init --preset <name>` bundle an agent with a hardware-appropriate
...
(`src/DEMON/learning/routing/complexity.py`). Produces a 0.0–1.0
...
**LLM-guided spec search** (`src/DEMON/learning/spec_search/`).
...
preset and a runnable tutorial at `examples/DEMON/spec_search_quickstart.py`.

**Six hybrid coordination paradigms** in `src/DEMON/agents/hybrid/`.
...
A runner CLI (`python -m DEMON.agents.hybrid.runner --cell <name>`)
...
- **agentskills.io spec adoption** — canonical `SKILL.md` format with YAML frontmatter following the [agentskills.io](https://agentskills.io/specification) open standard.
  ...
  - Sourced subdirectory layout (`~/.DEMON/skills/<source>/<name>/`)
...
- **Skills learning loop** — trace tagging, pattern discovery, DSPy/GEPA optimization.
  ...
  - `SkillOverlay` — sidecar storage at `~/.DEMON/learning/skills/<name>/optimized.toml`
...
- **Skills benchmark harness** — 4-condition PinchBench evaluation.
  ...
  - `DEMONAgentBackend` accepts `skills_enabled` and `overlay_dir` kwargs
  - Conditions: `no_skills`, `skills_on`, `skills_optimized_dspy`, `skills_optimized_gepa`

- **CLI commands:**
  - `DEMON skill list` / `info` / `run` / `install` / `sync` / `sources` / `update` / `remove` / `search`
  - `DEMON skill discover` — mine traces for recurring tool patterns
  - `DEMON skill show-overlay` — inspect optimization output
  - `DEMON optimize skills` — run DSPy/GEPA per-skill optimization
  - `DEMON bench skills` — run the PinchBench skills benchmark
...
- `examples/DEMON/spec_search_quickstart.py` — runnable end-to-end
...
- `src/DEMON/agents/hybrid/registry/*.toml` — 35-cell registry of
...
- **`learning/distillation/` is now `learning/spec_search/`.** The
  ...
  (`from DEMON.learning.distillation.*` →
  `from DEMON.learning.spec_search.*`). The `DEMON distillation`
...
- **Engine `generate_full` return shape extended.**
  `DEMONAgentBackend.generate_full` and `DEMONDirectBackend.generate_full`
...
- **OpenClaw category/name queries** — `DEMON skill install openclaw:owner/slug` now correctly splits into category + name match.
