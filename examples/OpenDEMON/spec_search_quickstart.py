...
are local fakes, so you can ``python examples/DEMON/spec_search_quickstart.py``
with no API keys and no Ollama / vLLM running. Every "swap this for production"
hookpoint is called out inline.

Configuration is read from ``configs/DEMON/examples/spec-search-quickstart.toml``
via the regular ``DEMON.core.config.load_config`` machinery — you can copy
that TOML to ``~/.DEMON/config.toml`` and tune the gate / stagnation /
reward knobs without editing this file.

Run:

    DEMON_HOME=/tmp/DEMON-spec-search-demo \\
...
from DEMON.learning.spec_search.multi_session import SpecSearchLoop
from DEMON.learning.spec_search.orchestrator import SpecSearchOrchestrator
...
def build_orchestrator(
    ...
    return SpecSearchOrchestrator.from_config(
        ...
        # Judge: production = DEMON.evals.core.scorer.LLMJudgeScorer(...)
        judge=MagicMock(),
...
def main() -> None:
    # In a real deployment, ``load_config()`` reads from ``~/.DEMON/config.toml``.
    ...
    #
    # (after copying ``configs/DEMON/examples/spec-search-quickstart.toml``
    # to ``~/.DEMON/config.toml``)
    ...
    home = Path(
        os.environ.get("DEMON_HOME")
        or tempfile.mkdtemp(prefix="DEMON-spec-search-")
    )
    print(f"DEMON_HOME = {home}")
    ...
    with patch(
        "DEMON.learning.spec_search.orchestrator.DiagnosisRunner"
    ) as MockDiag:
