...
```bash
uv run DEMON --version   # Should print 0.1.0
uv run DEMON --help      # Show all subcommands
```
...
DEMON uses [pytest](https://docs.pytest.org/) with approximately 1,000+
...
```bash
uv run pytest tests/ --cov=DEMON --cov-report=html
```
...
DEMON uses [Ruff](https://docs.astral.sh/ruff/) for linting, configured
in `pyproject.toml`:
...
The source code is organized under `src/DEMON/`:
...
src/DEMON/
    ...
    core/                       # Shared infrastructure
        config.py               # DEMONConfig, hardware detection, TOML loader
    ...
    cli/                        # Click CLI commands
        ...
        ask.py                  # DEMON ask
        init_cmd.py             # DEMON init
        model.py                # DEMON model list/info
        memory_cmd.py           # DEMON memory index/search/stats
        telemetry_cmd.py        # DEMON telemetry stats/export/clear
        bench_cmd.py            # DEMON bench run
        serve.py                # DEMON serve
```
...
try:
    import DEMON.memory.faiss_backend  # noqa: F401
except ImportError:
    pass
```
