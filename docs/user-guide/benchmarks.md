...
DEMON ships with two benchmarks:
...
DEMON bench run -n 50

# Run only the latency benchmark
DEMON bench run -b latency

# Run only the throughput benchmark with 20 samples
DEMON bench run -b throughput -n 20

# Specify model and engine
DEMON bench run -m qwen3:8b -e ollama

# Output JSON summary to stdout
DEMON bench run --json

# Write JSONL results to a file
DEMON bench run -o results.jsonl

# Combine options
DEMON bench run -b latency -n 100 -m qwen3:8b --json -o latency.jsonl
```
...
```bash
DEMON bench run -b context_length
```
