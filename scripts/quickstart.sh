...
uv run maturin develop -m rust/crates/DEMON-python/Cargo.toml --quiet 2>/dev/null \
...
uv run DEMON serve --port 8000 &>/dev/null &
CLEANUP_PIDS+=($!)
sleep 3
...
echo ""
echo -e "${GREEN}${BOLD}  DEMON is running!${NC}"
echo ""
