"""Regression tests for DEMON.mcp.loader.load_mcp_tools_from_config.

Closes the gap that #461 surfaced — MCP tools were silently dropped on
`DEMON ask` and `DEMON serve` because neither path read
...
def _mock_mcp_stack():
    ...
    with patch("DEMON.mcp.client.MCPClient") as MockClient, patch(
        "DEMON.mcp.transport.StreamableHTTPTransport"
    ) as MockHttp, patch(
        "DEMON.mcp.transport.StdioTransport"
    ) as MockStdio, patch(
        "DEMON.tools.mcp_adapter.MCPToolProvider"
    ) as MockProvider:
