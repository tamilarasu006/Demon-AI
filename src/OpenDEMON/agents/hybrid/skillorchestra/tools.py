...
both forced by the DEMON environment and documented inline:
...
* in-tool correctness check — the original ``answer`` tool LLM-judges the
  prediction against the gold answer inside ``call_tool``. DEMON
...
def run_answer(
    ...
    *,
    ...
    in-tool LLM correctness check is dropped — DEMON scores
    downstream.
    """
...
def run_search(
    ...
    *,
    ...
    DEMON substitution for the missing FAISS wiki index).
    """
    prompt = (
        context_str.strip() + "\n\n"
        + f"Question: {problem}\nInstead of directly answering the question, "
