...
A set of developer-focused scripts that use DEMON tool-using agents
to automate common coding tasks: code review, debugging, and test generation.

## What This Demonstrates

Each script wires up a `DEMON` instance with the `native_react` (ReAct) agent
...
1. **Install DEMON** (from the repo root):
...
```bash
python examples/code_companion/test_gen.py --module src/DEMON/tools/calculator.py
```
...
python examples/code_companion/test_gen.py \
    --module src/DEMON/tools/calculator.py \
    --framework unittest \
...
`memory_store`, `memory_search`, and more. Run `uv run DEMON eval list` or
inspect `src/DEMON/tools/` for the full registry.
