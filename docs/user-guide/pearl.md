...
DEMON includes a thin `DEMON pearl` wrapper for Pearl's native command
...
`DEMON pearl` looks for `pearld`, `oyster`, and `prlctl` on `PATH`, then under
`$PEARL_HOME/bin`.
...
```bash
DEMON pearl node -- --help
DEMON pearl wallet -- --help
DEMON pearl ctl -- --help
```

These map directly to:

| DEMON command | Pearl binary |
|---|---|
| `DEMON pearl node` | `pearld` |
| `DEMON pearl wallet` | `oyster` |
| `DEMON pearl ctl` | `prlctl` |

The command format is always `DEMON pearl <command>`. Pearl-native arguments
...
`DEMON mine` is the DEMON mining lifecycle. `DEMON pearl` is an escape
