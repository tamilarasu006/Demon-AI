...
use DEMON_core::{AgentContext, AgentResult, DEMONError, ToolResult};
use DEMON_tools::executor::ToolExecutor;
use regex::Regex;
    ...
    async fn run(
            ...
            .map(|ctx| {
                ctx.conversation
                    ...
                    .filter_map(|m| match m.role {
                        DEMON_core::Role::User => {
                            Some(RigMessage::user(&m.content))
                        }
                        DEMON_core::Role::Assistant => {
        ...
        for turn in 1..=self.max_turns {
            let response = self
                ...
                .await
                .map_err(|e| {
                    DEMONError::Agent(DEMON_core::error::AgentError::Execution(
                        e.to_string(),
                    ))
                })?;
...
mod tests {
    ...
    type OpenHandsAgent = NativeOpenHandsAgent<RigModelAdapter<DEMON_engine::Engine>>;
