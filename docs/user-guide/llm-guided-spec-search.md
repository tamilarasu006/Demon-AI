...
```bash
python examples/DEMON/spec_search_quickstart.py
```
...
`DEMON.learning.spec_search.composite_reward.score_batch`.
...
`configs/DEMON/examples/spec-search-quickstart.toml`. Copy it to
`~/.DEMON/config.toml` (or set `DEMON_CONFIG` to it) and the
...
| `trace_store` | `MagicMock` | `DEMON.traces.store.TraceStore(home / "traces.db")` |
| `student_runner` | `MagicMock` | `DEMON.learning.spec_search.student_runner.VLLMStudentRunner(host=..., model=...)` |
| `judge` | `MagicMock` | `DEMON.evals.core.scorer.LLMJudgeScorer(...)` (or a deterministic scorer if your benchmark provides one) |
| `session_store` | `MagicMock` | `DEMON.learning.spec_search.storage.session_store.SessionStore(home / "learning" / "sessions.db")` |
| `checkpoint_store` | `MagicMock` | `DEMON.learning.spec_search.checkpoint.store.CheckpointStore(home / "learning" / "checkpoints")` |
...
store = TraceStore("~/.DEMON/traces.db")
...
`src/DEMON/evals/backends/DEMON_agent.py` previously hardcoded
`builder.telemetry(telemetry).traces(True).build()`, ignoring the
`telemetry` parameter. This silently caused every agent-backend
evaluation to write to `~/.DEMON/traces.db` regardless of caller
...
- `examples/DEMON/spec_search_quickstart.py` — runnable end-to-end demo.
- `configs/DEMON/examples/spec-search-quickstart.toml` — prebuilt config.
- `src/DEMON/learning/spec_search/orchestrator.py` — `SpecSearchOrchestrator` (single session).
- `src/DEMON/learning/spec_search/multi_session.py` — `SpecSearchLoop` (Algorithm 1).
- `src/DEMON/learning/spec_search/composite_reward.py` — paper Eq. 1.
- `src/DEMON/learning/spec_search/gate/benchmark_gate.py` — `GateOK` predicate.
- `src/DEMON/learning/spec_search/external_adapter.py` — corpus → trace adapter.
