...
@click.group()
def daemon() -> None:
    """Manage the DEMON server daemon."""
...
def start(
    ...
    """Start the DEMON server as a background daemon."""
    console = Console(stderr=True)

    existing = _read_pid()
    if existing is not None:
        console.print(f"[yellow]Server already running (PID {existing}).[/yellow]")
        console.print("Use 'DEMON stop' to stop it first, or 'DEMON restart'.")
        sys.exit(1)
    ...
    # Build command to run DEMON serve
    cmd = [sys.executable, "-m", "DEMON.cli", "serve"]
    if host:
    ...
    console.print(
        f"[green]DEMON server started[/green] (PID {proc.pid})\n"
...
@daemon.command()
def stop() -> None:
    """Stop the running DEMON server daemon."""
    console = Console(stderr=True)
    pid = _read_pid()
    if pid is None:
...
def restart(ctx: click.Context) -> None:
    """Restart the DEMON server daemon."""
    console = Console(stderr=True)
    pid = _read_pid()
    if pid is not None:
...
@daemon.command()
def status() -> None:
    """Show status of the DEMON server daemon."""
    console = Console(stderr=True)
    pid = _read_pid()
    if pid is None:
