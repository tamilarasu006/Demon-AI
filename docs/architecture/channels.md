...
Channel implementations use Python's `threading` module rather than asyncio. This is a deliberate choice: DEMON's core inference path is synchronous, and daemon threads are simpler to compose with synchronous code than coroutines.
...
1. Create a new file in `src/DEMON/channels/`.
...
- [API Reference: Channels](../api-reference/DEMON/channels/index.md) -- complete class and type signatures
