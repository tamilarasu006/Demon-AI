...
//!
//! Rust translation of `src/DEMON/core/config.py`.
...
/// Default config directory: `~/.DEMON/`
pub fn default_config_dir() -> PathBuf {
    dirs_home().join(".DEMON")
}

/// Default config file path: `~/.DEMON/config.toml`
...
fn default_sandbox_image() -> String { "DEMON-sandbox:latest".into() }
...
fn default_operators_dir() -> String { "~/.DEMON/operators".into() }
...
pub fn load_config(path: Option<&Path>) -> Result<DEMONConfig, ConfigError> {
    ...
    let mut cfg = if config_path.exists() {
        ...
        let mut cfg: DEMONConfig = toml::from_str(&content)?;
...
mode = "block"
"#;
        let cfg: DEMONConfig = toml::from_str(toml_str).unwrap();
