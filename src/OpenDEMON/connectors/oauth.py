...
class OAuthProvider:
    ...
    # Filenames in ~/.DEMON/connectors/ to save tokens to
...
def get_client_credentials(
    ...
    Checks credential files in ``~/.DEMON/connectors/`` and falls
    back to environment variables ``DEMON_{NAME}_CLIENT_ID`` and
    ``DEMON_{NAME}_CLIENT_SECRET``.
    """
    ...
    prefix = f"DEMON_{provider.name.upper()}"
