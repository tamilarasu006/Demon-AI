...
# Engines supported by ``DEMON init --engine``.
_SUPPORTED_ENGINES = [
    "ollama",
    "vllm",
    "sglang",
...
def _next_steps_text(engine: str, model: str = "") -> str:
    ...
    steps: dict[str, str] = {
        "ollama": (
            ...
            '     DEMON ask "Hello"\n'
            "\n"
            "  Run `DEMON doctor` to verify your setup."
        ),
        "vllm": (
            "Next steps:\n"
            "\n"
...
def _quick_privacy_check(console: Console) -> None:
    ...
    console.print()
    console.print("  Run [cyan]DEMON scan[/cyan] for a full environment audit.")
...
@click.option(
    ...
    default=False,
    hidden=True,
    help="Run init non-interactively; called by the bare-DEMON first-run guard.",
)
@click.pass_context
def init(
    ctx: click.Context,
    ...
    if preset:
        examples_dir = (
            Path(__file__).resolve().parents[2] / "configs" / "DEMON" / "examples"
        )
        ...
        console.print(
            ...
            "[bold]DEMON doctor[/bold] to verify."
        )
        return
...
"""
        ...
        console.print(
            ...
            "Run [bold]DEMON connect gdrive[/bold] to connect "
            "Google services, then [bold]DEMON digest --fresh[/bold]."
        )
    ...
    if not soul_path.exists():
        soul_path.write_text(
            "# Agent Persona\n\nYou are DEMON, a helpful personal AI assistant.\n"
        )
        ...
        if not no_download and not from_bare_demon and spec:
            ...
            if click.confirm(prompt, default=True):
                ...
                console.print(
                    ...
                    f"    [bold]DEMON model pull {model}[/bold]"
                )
