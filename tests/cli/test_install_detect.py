...
def _patch_pkg_file(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    """Point ``DEMON.__file__`` at ``tmp_path / DEMON / __init__.py``."""
    pkg_dir = tmp_path / "DEMON"
...
def test_uv_tool_install_detected(tmp_path, monkeypatch):
    fake = tmp_path / "share" / "uv" / "tools" / "DEMON" / "lib" / "python3.12"
    fake.mkdir(parents=True)
    _patch_pkg_file(fake, monkeypatch)

    info = detect_install()
    assert info.kind == "uv-tool"
    assert info.upgrade_command == "uv tool upgrade DEMON"
...
def test_pypi_install_detected(tmp_path, monkeypatch):
    ...
    assert info.upgrade_command == "pip install --upgrade DEMON"
...
def test_missing_DEMON_file_falls_back_to_pypi(monkeypatch):
    """DEMON unimportable / no __file__ — still get a sane default."""
    with patch("DEMON.cli._install_detect.Path") as mock_path:
