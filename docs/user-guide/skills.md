...
| **Overlay** | Sidecar file at `~/.DEMON/learning/skills/` storing optimized descriptions and few-shot examples |
...
DEMON skill install hermes:apple-notes

# Bulk install a category
DEMON skill sync hermes --category research

# Run a skill directly
DEMON skill run math-solver -a expression="41 + 82"

# See skill details
DEMON skill info research-and-summarize
```
...
DEMON skill sync hermes --category research
DEMON skill sync hermes --category coding
DEMON skill sync hermes  # everything (~150 skills)
```
...
DEMON skill install openclaw:0xv4l3nt1n3/etherscan

# Bulk install with search filter
DEMON skill sync openclaw --search "web3|crypto"
```

### From Any GitHub Repo

```bash
DEMON skill install github:user/repo/path/to/skill --url https://github.com/user/repo
```
...
```bash
DEMON skill install github:Xquik-dev/hermes-tweet/skills/hermes-tweet --url https://github.com/Xquik-dev/hermes-tweet
```

### Config-Driven Auto Import

Add sources to `~/.DEMON/config.toml` for automatic syncing:
...
DEMON can automatically mine your trace history for recurring tool sequences and surface them as candidate skills:

```bash
# Preview discovered patterns without writing
DEMON skill discover --dry-run --min-frequency 3

# Write discovered skills to ~/.DEMON/skills/discovered/
DEMON skill discover
```

Discovered skills land in `~/.DEMON/skills/discovered/` and automatically appear in `DEMON skill list` on the next session.
...
DEMON optimize skills --dry-run

# Run DSPy optimization
DEMON optimize skills --policy dspy --min-traces 3

# Run GEPA evolutionary optimization
DEMON optimize skills --policy gepa --min-traces 3

# Inspect what optimization produced
DEMON skill show-overlay research-and-summarize
```

Optimization results are stored as sidecar overlays at `~/.DEMON/learning/skills/<skill-name>/optimized.toml`. They override the skill's description and add few-shot examples to the agent's system prompt. The original skill files are never modified.
...
# Single condition
DEMON bench skills --condition skills_optimized_dspy
```
...
|------|--------|-------------|---------|
| **Bundled** | Ships with DEMON | Implicit trust | Full access within declared capabilities |
...
```bash
DEMON skill install hermes:arxiv --with-scripts
```
...
skills_dir = "~/.DEMON/skills/"  # where skills are installed
...
2. **User** `~/.DEMON/skills/`
3. **Bundled** (shipped with DEMON)

## CLI Reference

| Command | Description |
|---------|-------------|
| `DEMON skill list` | List installed skills |
| `DEMON skill info <name>` | Show detailed skill information |
| `DEMON skill run <name> [-a key=value]` | Execute a skill directly |
| `DEMON skill install <source>:<name>` | Install from Hermes, OpenClaw, or GitHub |
| `DEMON skill sync [<source>] [--category C]` | Bulk install + update from sources |
| `DEMON skill sources` | List configured skill sources |
| `DEMON skill update` | Pull latest from configured sources |
| `DEMON skill remove <name>` | Remove an installed skill |
| `DEMON skill search <query>` | Search the skill index |
| `DEMON skill discover [--dry-run]` | Mine traces for recurring tool patterns |
| `DEMON skill show-overlay <name>` | Inspect optimization output for a skill |
| `DEMON optimize skills [--policy dspy\|gepa]` | Optimize skill descriptions + few-shot examples |
| `DEMON bench skills [--condition C]` | Run the PinchBench skills benchmark |
