...
through Pearl's vLLM miner and DEMON.

DEMON can point `vllm-pearl` at a model id, but a raw Hugging Face model is
...
DEMON only supports Pearl models published by the `pearl-ai` Hugging Face
organization. Private staging artifacts and DEMON-specific conversion
...
`DEMON mine start`, vLLM `/v1/models`, DEMON inference routing, Pearl
gateway template refresh, and `DEMON mine validate-model`.

`pearl-ai/Gemma-4-31B-it-pearl` and
`pearl-ai/Llama-3.1-8B-Instruct-pearl` are listed because they are public Pearl
org artifacts. They remain `planned` in DEMON until we have clean
...
2. Convert the target model.
   ...
   DEMON includes an experimental local converter for this work:
   ...
   its output as a staging artifact until `DEMON mine inspect-model` and
   `DEMON mine validate-model` pass on H100/H200 hardware.
...
3. Validate the Pearl vLLM plugin path.
   - Run `DEMON mine inspect-model --model <pearl-model-id>
...
4. Validate chain integration.
   ...
   - `DEMON mine status` parses those metrics.

5. Promote the model in DEMON.
...
```text
src/DEMON/mining/_models.py
```

`DEMON mine models` renders that registry. Planned models are visible to users
...
- `DEMON mine inspect-model --model <pearl-model-id> --allow-planned`
- `DEMON mine init --model <pearl-model-id>`
- `DEMON mine start`
- `curl http://127.0.0.1:8000/v1/models`
- `DEMON ask "Say hello in one sentence."`
- `DEMON mine status`
- `DEMON mine validate-model --model <pearl-model-id> --allow-planned --prompt
...
Attach the JSON artifact from `DEMON mine validate-model --output` to the issue.
