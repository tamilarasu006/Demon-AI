...
from DEMON.memory.sqlite import SQLiteMemory
...
```python
from DEMON.scheduler.tools import ScheduleTaskTool
from DEMON.scheduler.scheduler import TaskScheduler
from DEMON.scheduler.store import SchedulerStore

store = SchedulerStore(db_path="~/.DEMON/scheduler.db")
scheduler = TaskScheduler(store=store, system=DEMON_system)
scheduler.start()
...
```bash
DEMON ask --agent orchestrator --tools my_tool "Process this data"
```
...
# Multiple tools
DEMON ask --agent orchestrator --tools calculator,think "Solve: 2x + 5 = 13"

# All available tools (list them)
DEMON ask --agent orchestrator --tools calculator,think,retrieval,file_read "..."
```
