...
def test_init_accepts_from_bare_DEMON_flag(
    tmp_DEMON_home: Path, monkeypatch
) -> None:
    """The --from-bare-DEMON flag exists and suppresses the launch-chat prompt."""
    ...
    result = runner.invoke(
        init,
        ["--from-bare-DEMON", "--engine", "ollama", "--no-download", "--no-scan"],
        catch_exceptions=False,
    )
    assert result.exit_code == 0, result.output
    # When --from-bare-DEMON, we expect NOT to see a "launch chat" affordance.
...
def test_init_from_bare_DEMON_skips_engine_prompt(
    tmp_DEMON_home: Path, monkeypatch
) -> None:
    """--from-bare-DEMON must not hang on the engine-selection prompt
    ...
    result = runner.invoke(
        init,
        ["--from-bare-DEMON", "--no-download", "--no-scan"],
        catch_exceptions=False,
    )
    assert result.exit_code == 0, result.output
