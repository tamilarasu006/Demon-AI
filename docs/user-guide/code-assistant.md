...
This writes a pre-configured `~/.DEMON/config.toml` for the code assistant.
...
```bash
DEMON ask "Write a Python script that reads a CSV file and prints the top 5 rows"
```
...
DEMON ask "Write a Python script that parses JSON from stdin"

# Read and explain existing code
DEMON ask "Read main.py and explain the architecture"

# Fix a bug
DEMON ask "Find and fix the bug in test_utils.py"

# Run tests
DEMON ask "Run the test suite and summarize any failures"

# Explicitly specify agent and tools
DEMON ask --agent orchestrator --tools code_interpreter "Calculate the first 20 Fibonacci numbers"
...
The preset writes this to `~/.DEMON/config.toml`:
...
```bash
# Write a new script
DEMON ask "Write a Python script that converts YAML to JSON"

# Explain existing code
DEMON ask "Read src/DEMON/core/events.py and explain the EventBus pattern"

# Debug a failing test
DEMON ask "Run pytest tests/test_memory.py -v and fix any failures"

# Refactor code
DEMON ask "Read utils.py and refactor the parse_config function to use dataclasses"

# Generate tests
DEMON ask "Read src/DEMON/tools/calculator.py and write unit tests for it"

# Shell tasks
DEMON ask "Find all Python files larger than 100KB in this repo"
```
...
- The agent asks for confirmation before executing potentially destructive commands when running in interactive mode (`DEMON chat`).
- For stronger isolation, use the sandboxed agent: `DEMON ask --agent sandboxed --tools code_interpreter "..."`, which runs inside a Docker/Podman container.
...
**Shell command fails** -- The `shell_exec` tool runs commands relative to where you launched `DEMON`. Use `cd /path && command` in your prompt if needed, or run `DEMON` from the project directory.
