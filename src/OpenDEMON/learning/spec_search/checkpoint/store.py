...
class CheckpointStore:
    ...
    root :
        ...
        ``~/.DEMON/`` (or ``$DEMON_HOME``); for tests it's a
    ...
    def init(self) -> None:
        ...
        Refuses to initialize if ``self.root`` is inside the DEMON source
        ...
        if source_root is not None:
            try:
                ...
                raise ConfigurationError(
                    ...
                    f"DEMON source tree ({source_root}). Refusing to "
        ...
        if not (self._root / ".git").exists():
            self._git("init", "-q")
            self._git("config", "user.email", "spec-search@DEMON.local")
            self._git("config", "user.name", "DEMON Spec Search")
        ...
        # Allow empty so init succeeds even on a brand-new DEMON home.
        self._git(
            "commit",
            "--allow-empty",
            "-q",
