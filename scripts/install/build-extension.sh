...
#
# State files under $DEMON_HOME/.state/:
...
MANIFEST="$SRC_DIR/rust/crates/DEMON-python/Cargo.toml"
...
if uv run maturin develop -m "$MANIFEST" >>"$LOG" 2>&1; then
    ...
    if ! uv run python -c "import DEMON_rust" >>"$LOG" 2>&1; then
        rc=$?
        {
            echo "build-extension.sh: maturin succeeded but 'import DEMON_rust'"
