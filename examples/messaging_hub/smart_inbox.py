...
def _run_channel(channel: str, model: str, engine_key: str) -> None:
    ...
    help_text = setup_help.get(
        ...
        f"Run: DEMON channel list   to see available channels.\n",
    )
    ...
    click.echo("  from DEMON import DEMON")
    click.echo(f'  j = DEMON(model="{model}", engine_key="{engine_key}")')
    click.echo("  # Listen for incoming messages on the channel")
    click.echo(f'  # See: DEMON channel status  (to verify "{channel}" is connected)')
