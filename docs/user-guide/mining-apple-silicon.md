...
README. Then write DEMON' mining config:
...
```bash
DEMON mine doctor        # capability matrix
DEMON mine start         # launch gateway + miner-loop subprocesses
DEMON mine status        # check sidecar + gateway metrics
DEMON mine logs -n 120   # print recent logs
DEMON mine stop          # stop mining subprocesses
```
