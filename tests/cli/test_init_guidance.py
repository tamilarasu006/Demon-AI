...
class TestInitShowsNextSteps:
    def test_init_shows_next_steps(self, tmp_path: Path) -> None:
        ...
        config_dir = tmp_path / ".DEMON"
        config_path = config_dir / "config.toml"
        with (
            mock.patch("DEMON.cli.init_cmd.DEFAULT_CONFIG_DIR", config_dir),
            mock.patch("DEMON.cli.init_cmd.DEFAULT_CONFIG_PATH", config_path),
            mock.patch("DEMON.cli.init_cmd.PrivacyScanner"),
        ):
            result = CliRunner().invoke(cli, ["init", "--engine", "llamacpp", _NO_DL])
        assert result.exit_code == 0
        assert "Getting Started" in result.output
        assert "DEMON ask" in result.output
        assert "DEMON doctor" in result.output
...
class TestNextStepsOllama:
    def test_next_steps_ollama(self) -> None:
        ...
        assert "DEMON ask" in text
        assert "DEMON doctor" in text
...
class TestMinimalConfig:
    def test_init_generates_minimal_by_default(self, tmp_path: Path) -> None:
        """Default DEMON init produces a short config."""
        config_dir = tmp_path / ".DEMON"
        config_path = config_dir / "config.toml"
        with (
            mock.patch("DEMON.cli.init_cmd.DEFAULT_CONFIG_DIR", config_dir),
        ...
        assert "DEMON init --full" in content

    def test_init_full_generates_verbose_config(self, tmp_path: Path) -> None:
        """DEMON init --full produces the full reference config."""
        config_dir = tmp_path / ".DEMON"
        config_path = config_dir / "config.toml"
        with (
            mock.patch("DEMON.cli.init_cmd.DEFAULT_CONFIG_DIR", config_dir),
...
class TestInitEmptyModelFallback:
    def test_init_no_model_shows_warning(self, tmp_path: Path) -> None:
        ...
        with (
            mock.patch("DEMON.cli.init_cmd.DEFAULT_CONFIG_DIR", config_dir),
            mock.patch("DEMON.cli.init_cmd.DEFAULT_CONFIG_PATH", config_path),
            mock.patch("DEMON.cli.init_cmd.recommend_model", return_value=""),
            mock.patch("DEMON.cli.init_cmd.PrivacyScanner"),
        ):
...
class TestInitDownloadDispatch:
    def test_init_ollama_download_calls_ollama_pull(self, tmp_path: Path) -> None:
        ...
        with (
            ...
            mock.patch("DEMON.cli.init_cmd.DEFAULT_CONFIG_PATH", config_path),
            mock.patch(
                "DEMON.cli.init_cmd.ollama_pull",
                return_value=True,
            ) as mock_pull,
            mock.patch("DEMON.cli.init_cmd.PrivacyScanner"),
        ):
...
class TestInitPrivacyHook:
    def test_init_shows_privacy_summary(self, tmp_path: Path) -> None:
        ...
        with (
            ...
            result = CliRunner().invoke(cli, ["init", "--engine", "llamacpp", _NO_DL])
        assert result.exit_code == 0
        assert "DEMON scan" in result.output
