...
using DEMON. The assistant uses an orchestrator agent loop with web
...
- DEMON installed (`uv sync --extra dev` from the repo root)
...
recipe that can be loaded with `load_recipe()` or passed to the `DEMON eval`
runner.
...
User query
  |
  v
DEMON SDK  (model + engine selection)
  |
  v
...
- **Add more tools** -- append tool names to the `tools` list in
  `research.toml` or pass them on the command line. See `DEMON agent info
...
- **Use the recipe programmatically** -- load the TOML with
  `DEMON.recipes.load_recipe("examples/deep_research/research.toml")` and
...
- [Recipes](../../src/DEMON/recipes/) -- composable TOML configs that
