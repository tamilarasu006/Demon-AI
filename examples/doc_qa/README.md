...
Index a directory of documents into DEMON memory and answer questions
with context-augmented retrieval and citations.

## Requirements

- DEMON installed (`git clone https://github.com/DEMON/DEMON.git && cd DEMON && uv sync` or `uv sync --extra dev`)
...
1. **Index** -- Uses `DEMON.memory.index()` to chunk the documents at
...
DEMON SDK. Adjust `--chunk-size` and `--top-k` to tune the
