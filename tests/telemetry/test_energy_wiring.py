...
def _energy_config(tmp_path, gpu_metrics=True):
    """Build a DEMONConfig with energy monitoring enabled."""
    cfg = DEMONConfig()
    ...
    # `DEMON ask` (no --agent) now falls back to ``agent.default_agent``
...
class TestCliAskWiring:
    ...
    def test_energy_monitor_created_when_gpu_metrics_on(
        ...
        with patch(
            "DEMON.telemetry.energy_monitor.create_energy_monitor",
...
class TestSdkWiring:
    ...
    def test_engine_wrapped_in_ensure_engine(self):
        ...
        cfg = DEMONConfig()
        with patch(
            "DEMON.sdk.get_engine",
            return_value=("mock", engine),
        ):
            j = DEMON(config=cfg, model="test-model")
            j._ensure_engine()
...
class TestBenchWiring:
    ...
    def test_energy_monitor_passed_to_benchmarks(self):
        ...
        with (
            patch(
                "DEMON.cli.bench_cmd.get_engine",
                return_value=("mock", engine),
            ),
            patch(
                "DEMON.cli.bench_cmd.load_config",
                return_value=cfg,
            ),
            patch(
                "DEMON.telemetry.energy_monitor.create_energy_monitor",
...
def _patch_telemetry_config(tmp_path: Path):
    ...
    return mock.patch(
        "DEMON.cli.telemetry_cmd.load_config",
        return_value=cfg,
    ), db_path
