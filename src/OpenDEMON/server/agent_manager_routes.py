...
class _LightweightSystem:
    ...
    full DEMONSystem (which picks a random model from Ollama)."""
...
def _ensure_registries_populated() -> None:
    ...
    if not ChannelRegistry.keys():
        for mod_name in list(sys.modules):
            if mod_name.startswith("DEMON.channels.") and not mod_name.endswith(
                "_stubs"
            ):
                try:
