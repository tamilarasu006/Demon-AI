"""Tests for DEMON.learning.spec_search.checkpoint.store module."""
...
def _setup_isolated_repo_root(tmp_path: Path) -> Path:
    """Create a fake DEMON-home directory tree with config files for the
    CheckpointStore to track. Returns the root."""
    root = tmp_path / "DEMON_home"
...
class TestCheckpointStoreInit:
    ...
    def test_init_refuses_inside_source_tree(
        ...
        from OpenDEMON.learning.spec_search.storage import paths

        source_root = paths._find_source_root()
        assert source_root is not None
        bad_root = source_root / "fake_DEMON_home"
