...
def install(query: str, with_scripts: bool, force: bool, url: str):
    ...
    Example: ``DEMON skill install hermes:apple-notes``
    """
    console = Console()
...
def remove(skill_name: str, yes: bool):
    ...
    Searches ``~/.DEMON/skills/`` and ``./skills`` for a directory whose
...
def search(query: str, source: str):
    ...
    console.print(
        ...
        f"Install with: DEMON skill install <source>:<name>[/dim]"
    )
...
def discover(min_frequency: int, min_outcome: float, dry_run: bool) -> None:
    ...
    discovered skill manifests under ~/.DEMON/skills/discovered/."""
    console = Console()
    ...
    if dry_run:
        ...
        tmp = Path(tempfile.mkdtemp(prefix="DEMON-discover-dryrun-"))
        try:
