...
@click.group()
def compose() -> None:
    """Compose, run, benchmark, and deploy DEMON configurations.
...
def compose_list(kind: Optional[str]) -> None:
    ...
    try:
        ...
        if not recipes:
            ...
            console.print(
                "[dim]Place TOML recipes in src/DEMON/recipes/data/ "
                "or ~/.DEMON/recipes/[/dim]"
            )
            return
