...
use DEMON_core::error::{DEMONError, ToolError};
use DEMON_core::{EventBus, EventType, ToolResult};
use DEMON_security::capabilities::CapabilityPolicy;
use DEMON_security::taint::{TaintSet, check_taint};
...
impl ToolExecutor {
    ...
    pub fn execute(
    ...
    ) -> Result<ToolResult, DEMONError> {
        let tool = self.tools.get(tool_name).ok_or_else(|| {
            DEMONError::Tool(ToolError::NotFound(tool_name.to_string()))
        })?;
        ...
        if let (Some(policy), Some(aid)) = (&self.capability_policy, agent_id) {
            ...
            for cap in &spec.required_capabilities {
                if !policy.check(aid, cap, "") {
                    return Err(DEMONError::Tool(ToolError::CapabilityDenied(
                        aid.to_string(),
        ...
        if let Some(taint_set) = taint {
            if let Some(violation) = check_taint(tool_name, taint_set) {
                return Err(DEMONError::Tool(ToolError::TaintViolation(
                    tool_name.to_string(),
                    violation,
                )));
            }
        ...
        if elapsed > timeout {
            ...
            return Err(DEMONError::Tool(ToolError::Timeout(
                timeout.as_secs_f64(),
                tool_name.to_string(),
            )));
        }
...
mod tests {
    ...
    fn test_executor_tool_not_found() {
        ...
        let err = exec
            ...
            .unwrap_err();
        assert!(matches!(err, DEMONError::Tool(ToolError::NotFound(_))));
    }
}
