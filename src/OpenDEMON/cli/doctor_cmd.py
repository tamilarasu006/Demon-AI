...
def _check_config_exists() -> CheckResult:
    ...
    return CheckResult(
        ...
        details="Run `DEMON init` to generate a config file.",
    )
...
def _check_optional_deps() -> List[CheckResult]:
    ...
    results: List[CheckResult] = []
    optional_packages = [
        ("fastapi", "DEMON[server]", "REST API server"),
        ("torch", "pip install torch", "SFT/GRPO training"),
        ("pynvml", "DEMON[gpu-metrics]", "NVIDIA energy monitoring"),
        ("amdsmi", "DEMON[energy-amd]", "AMD energy monitoring"),
        ("colbert", "DEMON[memory-colbert]", "ColBERT memory backend"),
        ("zeus", "DEMON[energy-apple]", "Apple Silicon energy monitoring"),
    ]
...
def doctor(as_json: bool) -> None:
    """Run diagnostic checks on your DEMON installation."""
    ...
    console = Console()
    console.print()
    console.print("[bold]DEMON Doctor[/bold]")
    console.print()
