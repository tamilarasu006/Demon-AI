...
This tutorial demonstrates how to use DEMON to run **autonomous scheduled agents** that perform recurring personal tasks on a cron-like schedule.

## What This Demonstrates

- Using the `DEMON` SDK with different agent types (`orchestrator`, `native_react`) and tool sets
- Configuring recurring schedules via TOML and the `DEMON scheduler` CLI
...
Register each script as a recurring task with `DEMON scheduler create`:
...
uv run python examples/scheduled_ops/daily_digest.py | DEMON channel send slack
...
See `DEMON channel list` for all available channels.
...
- **Model selection**: Use `--model` to target a specific model, or let DEMON auto-select from what is available.
