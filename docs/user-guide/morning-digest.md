...
# Spotify
DEMON connect spotify

# Strava
DEMON connect strava
```
...
echo '{"api_key": "YOUR_KEY", "location": "San Francisco,CA,US"}' > ~/.DEMON/connectors/weather.json

# GitHub notifications (token from https://github.com/settings/tokens)
echo '{"token": "ghp_YOUR_TOKEN"}' > ~/.DEMON/connectors/github.json

# News RSS (no auth needed — configure your feeds)
cat > ~/.DEMON/connectors/news_rss.json << 'EOF'
{"feeds": [
...
```bash
CARTESIA_API_KEY="sk_car_..." DEMON digest --fresh
```
...
```bash
DEMON digest --fresh          # Generate a new digest now
DEMON digest                  # Show today's cached digest
DEMON digest --text-only      # Print text without audio
DEMON digest --history        # Show past digests
DEMON digest --schedule "0 6 * * *"   # Set daily schedule
DEMON digest --schedule off   # Disable schedule
DEMON digest --schedule       # Show current schedule
```

## Saying "Good morning"

When chatting with DEMON (via CLI, desktop, or browser), saying "Good morning" or "morning digest" automatically triggers the digest — no need to use the `digest` command explicitly.
...
The `persona` field loads a prompt file from `configs/DEMON/prompts/personas/{name}.md`. The default `DEMON` persona delivers briefings with dry British wit, prioritizes urgent items, and interprets health data as trends rather than raw numbers.

To create a custom persona, add a new `.md` file in the personas directory.

### News Feeds

Add any RSS or Atom feed to `~/.DEMON/connectors/news_rss.json`:
...
**"No digest for today"** — Run `DEMON digest --fresh` to generate one.

**Empty sections** — Check connector status with `DEMON connect --list`. Ensure tokens haven't expired (Google/Spotify tokens expire after 1 hour and are auto-refreshed on next use).
