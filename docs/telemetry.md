...
DEMON ships **anonymous usage telemetry** by default so the team can
...
- **Self-hosted backend** on the DEMON team's PostHog instance —
...
[`src/DEMON/analytics/events.py`](../src/DEMON/analytics/events.py).
...
1. [`src/DEMON/analytics/redaction.py`](../src/DEMON/analytics/redaction.py) — value-level pattern matching (20+ regexes for PII).
2. [`src/DEMON/analytics/events.py`](../src/DEMON/analytics/events.py) — structural allowlist (event name + property name + type validator).
...
- The leaderboard / contest opt-in (`OptInModal.tsx`) is a separate,
  ...
  the DEMON leaderboard. It is **not** the same as analytics and
